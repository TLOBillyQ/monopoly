...
当前代码中 `src/game/core/runtime/player_state/StatusOps.lua` 仍直接 `require("src.core.RuntimeCompat")`，并在 `set_player_seat` / `stop_all_players_movement` 中调用 `get_vehicle_helper()`；这是 R10 第一优先级风险点。证据：`StatusOps.lua` 第 3、22、64 行。

`runtime_context.install_runtime_helpers(ctx, opts)` 当前默认 `install_globals = true`，并且测试已固化该默认行为，因此 M41 不是“只改一行默认值”，必须同步调整调用点与测试预期。证据：`src/core/RuntimeContext.lua` 第 251-254 行，`tests/suites/gameplay.lua` 第 675-678 行。

R9 虽已拆分部分 turn 逻辑，但 `GameplayLoop.lua`（291 行）与 `GameplayLoopRuntime.lua`（212 行）仍是高认知负担文件，继续拆分应优先选择“纯编排段”而非引入新语义。证据：对应文件当前行数与函数分布。
...
- `src/game/core/runtime/player_state/StatusOps.lua`：当前 game core 中唯一直接依赖 `RuntimeCompat` 的状态操作模块。
...
- `src/core/RuntimeContext.lua`：`install_runtime_helpers` 默认会写 legacy globals，M41 将调整默认值。
