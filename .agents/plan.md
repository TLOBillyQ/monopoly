...
根据 `research.md` 最新 R4 复评，现追加第五轮 M19-M21：聚焦 `presentation -> game/systems` 直连清理、`UIModel/UIIntentDispatcher` 按用例切片、以及依赖规则白名单递减治理。该轮已完成实现并通过回归验证。
...
- [x] (2026-03-01 15:21Z) M19：已引入 `src/presentation/read_model/GameplayReadPort.lua`，替换 `MoveAnim/placement/UIPanel` 对 `VehicleFeature/LandPricing` 的直连，`presentation -> game/systems` 命中清零。
- [x] (2026-03-01 15:21Z) M20：已将 `UIModel` 与 `UIIntentDispatcher` 按用例拆分为子模块切片，保留原入口 API 并完成行为等价验证。
- [x] (2026-03-01 15:21Z) M21：已在 `dep_rules` 增加 `presentation -> game/systems` 白名单递减守护（含 stale whitelist 失败机制）。
...
实施新增发现 15（执行 M19）：`VehicleFeature` 与 `LandPricing` 在 presentation 侧只使用只读语义（seat 解析与地块投入估值），可稳定下沉到 adapter 读端口而不影响领域规则。

实施新增发现 16（执行 M20-M21）：`UIModel` 与 `UIIntentDispatcher` 以切片目录拆分后，主入口体积显著下降且对外签名不变；`dep_rules` 白名单递减机制可同时阻止新增依赖和遗留白名单长期滞留。
...
- 决策：M19 采用“presentation 读端口”而非直接把 `game/systems` 模块上提到 core。
  理由：本轮目标是边界收口且不改行为；读端口可最小化改动面并保留后续再抽象空间。
  日期/作者：2026-03-01 / Codex GPT-5。
...
第五轮 M19-M21 已执行完成：`presentation -> game/systems` 直连清零；`UIModel` 与 `UIIntentDispatcher` 已按用例切片拆分并保持入口兼容；`dep_rules` 已启用 `presentation -> game/systems` 白名单递减守护（含 stale 项失败）。回归基线保持 `All regression checks passed (190)` 且 `dep_rules ok / tick ok / forbidden_globals ok`。本计划状态更新为“全部完成”。
...
- 修订：执行并回填 M19-M21（adapter 读端口替换、UIModel/UIIntentDispatcher 切片拆分、dep_rules 白名单递减守护）。
  原因：用户要求“执行计划”。
  日期/作者：2026-03-01 / Codex GPT-5。
