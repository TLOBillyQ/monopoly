...
- [ ] 里程碑 1：建立向内的意图输出端口，并用一条最小业务路径验证 `systems` 可以在不依赖 `flow.intent` 的前提下打开 choice / popup。
- [ ] 里程碑 2：并行迁移 `land / effects / choice handlers`、`items`、`market` 三组业务模块，彻底切断 `src/game/systems -> src/game/flow.*` 依赖，然后收紧静态守护。
- [ ] 里程碑 3：让 `state.ui_runtime` 成为唯一 UI 运行时事实源，逐步退役 `LegacyOutputMirror` 与对 root-level `state.*` 的直接读写。
- [ ] 里程碑 4：删除 `ui_port` 兼容回退，强制使用 `anim_gate_port`、`popup_port`、`tile_feedback_port`、`board_scene_port`。
- [ ] 里程碑 5：在行为边界稳定后，再迁移 runtime 具体实现离开 `src/core`，同步文档和守护规则，并跑最终全量回归。
...
id：T1。depends_on：[]。status：Not Completed。location：`src/game/ports/IntentOutputPort.lua`、`src/game/flow/ports/IntentOutputAdapter.lua`、`src/game/flow/intent/IntentDispatcher.lua`、`src/game/flow/turn/GameplayLoop.lua`、`src/game/systems/land/LandingPresenter.lua`、新增 `tests/suites/intent_output_contract.lua`。
...
log：实施时填写，至少记录探针选择了哪条路径、为什么选它、是否需要修改 adapter 形状。

files edited/created：实施时填写。

### T2 迁移 `land / effects / choice handlers` 领域脱离 `flow.intent`

id：T2。depends_on：[T1]。status：Not Completed。location：`src/game/systems/effects/EffectPipeline.lua`、`src/game/systems/choices/ChoiceHandlers/`、`src/game/systems/land/`、必要时补充 `tests/suites/land.lua`、`tests/suites/landing.lua`、`tests/suites/cross_module_contract.lua`、`tests/suites/presentation_ui.lua`。
...
log：实施时填写，记录哪几个 handler 需要扩展 payload，哪几个只做了简单替换。

files edited/created：实施时填写。

### T3 迁移 `items` 领域脱离 `flow.intent`

id：T3。depends_on：[T1]。status：Not Completed。location：`src/game/systems/items/ItemInventory.lua`、`src/game/systems/items/ItemUseBroadcast.lua`、`src/game/systems/items/ItemPhase.lua`、相关测试 `tests/suites/item.lua`、`tests/suites/gameplay_core.lua`、`tests/suites/presentation_ui_interaction.lua`。
...
log：实施时填写，记录 item slot、auto flow 和 broadcast 哪些地方最容易被新端口形状影响。

files edited/created：实施时填写。

### T4 迁移 `market` 领域脱离 `flow.intent`

id：T4。depends_on：[T1]。status：Not Completed。location：`src/game/systems/market/service/ChoiceOutcome.lua`、必要时连带 `src/game/systems/market/service/ChoiceSession.lua`、`src/game/systems/market/MarketService.lua`，以及测试 `tests/suites/market.lua`、`tests/suites/paid_currency.lua`、`tests/suites/presentation_ui_popup_market.lua`。
...
log：实施时填写，记录 market paid callback 是否需要额外 adapter，还是沿用现有 gateway 即可。

files edited/created：实施时填写。

### T5 收紧 `dep_rules` 并把 `systems -> flow` 反向依赖纳入永久守护

id：T5。depends_on：[T2, T3, T4]。status：Not Completed。location：`tests/internal/dep_rules.lua`、必要时补充 `tests/suites/architecture_guard_contract.lua`、`tests/suites/usecase_boundary_contract.lua`。
...
log：实施时填写，记录守护是如何收紧的，是否存在必须显式豁免的历史文件；如果有豁免，必须写出退出条件。

files edited/created：实施时填写。

### T6 给 `RuntimeState` 增加显式读写 API，并先迁移 `game/flow` 消费方

id：T6。depends_on：[]。status：Not Completed。location：`src/core/RuntimeState.lua`、`src/game/flow/ports/UseCaseOutputPort.lua`、`src/game/flow/turn/TickUISync.lua`、`src/game/flow/turn/TurnDispatchValidator.lua`、`src/game/flow/turn/GameplayLoopTickSteps.lua`、新增 `tests/suites/ui_runtime_state_contract.lua`，以及按需最小修改 `tests/suites/ui_gate_contract.lua`。
...
log：实施时填写，记录新增了哪些 `RuntimeState` 读写函数，以及哪些 flow 消费方因此变成只读 `ui_runtime`。

files edited/created：实施时填写。

### T7 迁移 `presentation` 和 bootstrap 对 UI 运行时状态的消费

id：T7。depends_on：[T6]。status：Not Completed。location：`src/presentation/api/presentation_ports/ui_sync/UIModelSync.lua`、`src/presentation/interaction/UIModalStateCoordinator.lua`、`src/presentation/ui/UIModalPresenter.lua`、`src/presentation/interaction/ui_intent_dispatcher/PreConfirmFlow.lua`、`src/app/bootstrap/GameStartupEventBridge.lua`、必要时补充 `src/presentation/state/ui_model/ChoiceSlice.lua` 和相关测试。
...
log：实施时填写，记录哪些 presentation 模块最依赖旧状态布局，是否需要在 `RuntimeState` 再补一个只读 helper 才能保持代码整洁。

files edited/created：实施时填写。

### T8 退役 `LegacyOutputMirror` 并把 root-level legacy 状态降为只读兼容或彻底删除

id：T8。depends_on：[T7]。status：Not Completed。location：`src/game/flow/ports/LegacyOutputMirror.lua`、`src/game/flow/ports/UseCaseOutputPort.lua`、`src/core/RuntimeState.lua`、新增 `tests/suites/legacy_output_mirror_contract.lua`，以及相关回归 `tests/suites/presentation_ui.lua`。
...
log：实施时填写，记录 `LegacyOutputMirror` 是被删除、保留空壳，还是被改成启动期单向迁移工具；必须写清原因。

files edited/created：实施时填写。

### T9 删除 `ui_port` 兼容回退并强制窄端口安装

id：T9。depends_on：[]。status：Not Completed。location：`src/game/core/runtime/Game.lua`、`src/core/ActionAnimPort.lua`、`src/game/flow/turn/TurnRoll.lua`、`src/game/flow/turn/TurnMove.lua`、新增 `tests/suites/narrow_runtime_ports_contract.lua`，以及相关回归 `tests/suites/presentation_ui_action_anim.lua`、`tests/suites/gameplay_loop.lua`。
...
log：实施时填写，记录哪些文件被真正迁走，哪些因为 façade 策略暂时保留在 `src/core`，以及下一次删除 shim 的条件。

files edited/created：实施时填写。

### T11 同步文档、最终守护和收尾验收

id：T11。depends_on：[T10]。status：Not Completed。location：`docs/architecture/boundaries.md`、`.agents/research.md`、`.agents/plan.md`、必要时补充 `tests/internal/dep_rules.lua` 和相关契约测试。
...
log：实施时填写，记录文档同步了哪些事实，以及哪些后续问题被明确留给下一轮。
