...
- [ ] `T4` 进行中：presentation 热点已派工，等待 `play_mine_trigger` / `_resolve_tile_index` / turn label 命名化结果。
...
`src/rules/` 承载规则与效果计算，是 CRAP 热点和配置聚合问题的集中区。`src/turn/` 承载回合状态机与等待逻辑，既有热点函数，也有 wait 层重复 helper。`src/presentation/` 与 `src/ui/` 承担宿主事件桥接、界面渲染和 UI runtime 维护，是入口切换和 landing visual hold 收口的核心区域。`src/state/` 负责集中式游戏状态与访问器，是 landing visual hold 单源化和 `game_state` 组装保护的落点。`src/app/bootstrap/` 目前仍控制启动编排，后续要改成显式入口，并把宿主入口最终切到 `src/presentation/runtime/install.lua`。
...
以下命令全部在仓库根目录 `/Users/billyq/Dev/Github/Lua/monopoly` 执行，并且已经成功跑通：
...
Wave 0 只有 `T0`，用于确立基线和重写计划。Wave 1 处理热点、死代码审计、入口显式化原型和 landing visual hold 原型门。Wave 2 只在第一波结果稳定后处理共享 helper 合并与入口反转切换。Wave 3 一次性完成 `gameplay/rules.lua` 消费者迁移以及 hold caller 切换。Wave 4 重跑 baseline，核对分数、守卫和风险偏差。
