...
- [ ] M13：清理 clock 兼容别名（`now/diff_seconds`）与 fallback 调用路径，仅保留 wall/cpu 显式语义。
- [ ] M14：退役 state 顶层兼容字段镜像，统一迁移到 `ui_runtime/board_runtime/anim_runtime/turn_runtime/debug_runtime` 命名空间。
- [ ] M15：收紧 `ui_runtime_port` 兼容代理，实现显式契约并移除动态字段透传。
