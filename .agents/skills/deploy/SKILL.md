...
2. **确定目标路径** - 按以下优先级：
   ...
   - 默认路径：
     - Windows: `~/Desktop/dev/LuaSource_大富翁`
     - macOS: `~/Documents/eggy/LuaSource_大富翁`
3. **拷贝目录** - 复制 `src/` 和 `vendor/third_party/` 到目标目录
4. **拷贝文件** - 复制 `main.lua`、`Data/UIManagerNodes.lua`、`Data/Prefab.lua`
5. **自动决定模式** - 不带 `--startup-profile` 时按 `release` 部署；带 `--startup-profile` 时自动按 `debug` 部署
6. **可选注入 Profile** - 如指定 `--startup-profile`，会在 main.lua 开头注入 `STARTUP_TEST_PROFILE` 变量
7. **统计代码行数** - 计算有效 Lua 代码行数（不含空行和注释）
...
> 指定 `--startup-profile` 时，部署脚本会自动切到 `debug` 模式。
