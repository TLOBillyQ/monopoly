...
- [x] (2026-01-28 15:40Z) 完成骰子 UI 移除与文档/审计清单同步（`ui_panel`/`eggy_layer_ui`/`docs/ui`/`tests/ui_nodes_audit`）。
...
随后改骰子 roll：在 `src/gameplay/turn_roll.lua` 里计算出 `rolls/total` 后，如果 `game.ui_port` 标记支持 `wait_action_anim`，写入 `turn.action_anim`（kind=roll，包含玩家与点数信息），返回 `wait_action_anim` 并保留 `resume_state` 与 `resume_args`；同时删除 `ui_panel.lua` 里 `dice_text` 的构建与 `eggy_layer_ui.lua` 里对 `panel_current_dice` 的写入，更新 `docs/ui/02_base_screen.md`、`docs/ui/01_canvas_inventory.md`、`docs/ui/ui_naming_list.md` 与 `tests/ui_nodes_audit.lua` 去除该节点。
...
在仓库根目录先定位涉及骰子与道具展示的代码和文档：
    ...
    rg -n "panel_current_dice|dice_text|push_popup|roadblock|mine|missile|monster|clear_obstacles" src docs tests Data -S
...
自动化验证：在仓库根目录执行 `lua tests/ui_nodes_audit.lua`，确认 UI 节点清单不再包含 `panel_current_dice` 且审计通过；然后执行 `lua tests/deps_check.lua` 与 `lua tests/regression.lua`，预期全部通过。
...
以上接口需与现有 `move_anim` 的使用方式保持一致，并在 `docs/adapters_design.md` 中同步说明。
