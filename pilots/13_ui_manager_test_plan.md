...
- [ ] (2026-01-30 13:59) 梳理 UIManager 实现与文档对照点，记录必要差异（已完成：阅读实现并确认测试范围；剩余：按文档逐项核对差异）
- [x] (2026-01-30 13:59) 编写并跑通 `tests/ui_manager_test.lua`
- [ ] (2026-01-30 13:59) 完成 Eggitor 手动测试并记录可观察结果

## 意外与发现


- 观察：`Library/UIManager` 下未发现 `Promise.lua`，原计划中的 Promise 覆盖无法直接落地。
  证据：`rg -n "Promise" Library/UIManager` 无输出。
...
已新增 `tests/ui_manager_test.lua` 并跑通，覆盖 UIManager 的构建、查询、属性同步、事件监听与 Frameout。仍缺 Eggitor 手动测试与文档逐项对照。
...
2026-01-30 更新：新增 `tests/ui_manager_test.lua` 并跑通，记录缺失 Promise 模块的发现与测试覆盖调整。原因是计划需反映实际仓库现状。
