...
- [x] (2026-01-28 18:55Z) 生成 Lua 环境合规扫描报告并落盘（`docs/eggy/lua_env_audit.md`）。
...
- 观察：`src/` 目录未再发现 `io/os/package/debug`、`math.random` 等违规用法。
  证据：`lua tests/lua_env_audit.lua` 输出 `ok: no violations in src`。
...
已完成合规扫描与修复，新增审计脚本与报告。代码侧移除 `io/os/package/debug`、`math.random` 依赖，并通过 `lua tests/lua_env_audit.lua`、`lua tests/deps_check.lua`、`lua tests/regression.lua` 验证无违规与回归通过。后续如新增 Lua 文件需保持审计脚本通过。
