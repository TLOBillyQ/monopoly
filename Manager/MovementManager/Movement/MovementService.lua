...
function MovementService.move(game, player, steps, opts)
  ...
  for step = 1, abs_steps do
    ...
    if board:has_roadblock(current) then
      ...
      emit_event(MonopolyEvent.movement.roadblock_hit, {
        player = player,
    ...
    if not opts.skip_steal_check and #encountered_step > 0 then
      ...
      if has_steal and remaining > 0 then
        ...
        emit_event(MonopolyEvent.movement.steal_interrupt, {
          player = player,
          encountered_ids = encountered_step,
    ...
    if steps > 0 and not opts.skip_market_check then
      ...
      if tile.type == "market" and step < steps then
        ...
        emit_event(MonopolyEvent.movement.market_interrupt, {
          player = player,
  ...
  emit_event(MonopolyEvent.movement.moved, {
    player = player,
  ...
  if pass_start > 0 then
    ...
    emit_event(MonopolyEvent.movement.passed_start, {
      player = player,
      count = pass_start,
      bonus = bonus,
