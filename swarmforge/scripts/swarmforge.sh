...
parse_config() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    case "$agent" in
      claude|codex|copilot|grok|kimi) ;;
      *)
...
check_helper_scripts() {
  local helper
  for helper in handoff-lib.sh swarm_handoff.sh swarm_handoff.bb ready_for_next.sh ready_for_next.bb done_with_current.sh done_with_current.bb ready_for_next_task.sh ready_for_next_task.bb done_with_current_task.sh done_with_current_task.bb ready_for_next_batch.sh ready_for_next_batch.bb done_with_current_batch.sh done_with_current_batch.bb handoffd.bb swarm-cleanup.sh swarm-window-watchdog.sh swarm-terminal-adapter.sh; do
...
launch_role() {
  ...
  case "$agent" in
    claude)
      launch_cmd="export SWARMFORGE_ROLE='$role' && export PATH='$role_script_dir':\$PATH && cd '$role_worktree' && claude --append-system-prompt-file '$prompt_file' --dangerously-skip-permissions -n 'SwarmForge ${display}' \"\$(cat '$prompt_file')\""
      ;;
    codex)
      launch_cmd="export SWARMFORGE_ROLE='$role' && export PATH='$role_script_dir':\$PATH && cd '$role_worktree' && codex -C '$role_worktree' --yolo \"\$(cat '$prompt_file')\""
      ;;
    copilot)
      launch_cmd="export SWARMFORGE_ROLE='$role' && export PATH='$role_script_dir':\$PATH && cd '$role_worktree' && copilot -C '$role_worktree' --name 'SwarmForge ${display}' --yolo -i \"\$(cat '$prompt_file')\""
      ;;
    grok)
      launch_cmd="export SWARMFORGE_ROLE='$role' && export PATH='$role_script_dir':\$PATH && cd '$role_worktree' && grok --cwd '$role_worktree' --dangerously-skip-permissions --rules \"\$(cat '$prompt_file')\""
      ;;
    kimi)
      launch_cmd="export SWARMFORGE_ROLE='$role' && export PATH='$role_script_dir':\$PATH && cd '$role_worktree' && kimi --yolo --add-dir '$role_worktree'"
      ;;
  esac
  ...
  if [[ "$agent" == "kimi" ]]; then
    send_initial_kimi_prompt "$session" "$display" "$prompt_file"
  fi
