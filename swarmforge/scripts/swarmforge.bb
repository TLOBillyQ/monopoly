...
(defn send-initial-kimi-prompt! [ctx index row prompt-file]
  ...
  (let [target (tmux-agent-target (:display-name row) (:tmux-pane-base-index ctx) (:session row))
        base-delay (env-long "SWARMFORGE_KIMI_INJECT_DELAY_SECONDS" 1)
        delay-seconds (+ base-delay index)
...
(defn parse-agent-override [args]
  (cond
...
(defn test-agent-override! [args]
  (let [[agent-override root] (parse-agent-override args)]
    (println (str "{:agent-override " (pr-str agent-override) " :root " (pr-str root) "}"))))
...
(defn -main [& args]
  (case (first args)
    ...
    "--test-launch-command" (apply test-launch-command!
                                     ...
                                     (drop 2 args))
    "--test-agent-override" (test-agent-override! (rest args))
