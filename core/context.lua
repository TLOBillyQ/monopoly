...
local env_bindings = require("core.env")
local editor = require("core.editor")
local vehicle_feature = require("game.vehicle")
require("cfg.RuntimeRefs")
...
function context.set_current(ctx)
  current_context = ctx
  return ctx
end
...
function context.refresh_roles(ctx)
...
function context.install_globals(ctx)
  assert(ctx ~= nil and ctx.env ~= nil, "missing runtime context")
  context.install_environment(ctx)
  context.install_runtime_helpers(ctx)
  context.install_editor_exports(ctx)
end

function context.install_environment(ctx)
  assert(ctx ~= nil and ctx.env ~= nil, "missing runtime context")
  env_bindings.install(ctx.env)
end

function context.install_runtime_helpers(ctx)
