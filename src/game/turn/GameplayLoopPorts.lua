...
local function _default_refresh_from_dirty(game, state, dirty)
  ...
  if dirty.any or dirty.ui then
    ...
    if only_countdown then
      ...
      local runtime_constants = tick_ui_sync.runtime_constants()
...
local function _default_install_event_handlers(game, logger, state)
  local event_handlers = require("src.presentation.api.UIEventHandlers")
  event_handlers.install(game, logger, state)
end

local function _default_on_bankruptcy_tiles_cleared(game, player, owned_tile_ids)
