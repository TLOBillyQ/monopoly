...
function property_query.resolve_rent_owner(game, tile, state_fn)
  local st = property_query.safe_tile_state(game, tile)
  if state_fn then
...
function property_query.contiguous_rent(game, board, index, owner_id)
  ...
  local start_state = property_query.safe_tile_state(game, start_tile)
  ...
  local rent_sum, component = rent_math.compute_contiguous_rent(start_tile.id, owner_id, land_neighbors,
    function(tile_id)
      ...
      local st2 = property_query.safe_tile_state(game, tile)
