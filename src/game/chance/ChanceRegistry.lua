...
local function _apply_cash_change(player, delta)
  player:add_cash(delta)
end

local function _adjust_chance_delta(player, delta)
...
local function _apply_cash_and_maybe_bankrupt(game, player, delta)
  _apply_cash_change(player, delta)
    ...
    if card.target == "all" then
      for _, p in ipairs(game.players) do
        if not p.eliminated then
          local delta = _adjust_chance_delta(p, card.amount)
          _apply_cash_change(p, delta)
      ...
      local delta = _adjust_chance_delta(player, card.amount)
      _apply_cash_change(player, delta)
      _emit_event(monopoly_event.chance.applied, {
        player = player,
        card = card,
    ...
    if card.target == "all" then
      for _, p in ipairs(game.players) do
        if not p.eliminated then
          ...
          local delta = _adjust_chance_delta(p, -fee)
          _apply_cash_and_maybe_bankrupt(game, p, delta)
          _emit_event(monopoly_event.chance.applied, {
            player = p,
            card = card,
      ...
      local delta = _adjust_chance_delta(player, -fee)
      _apply_cash_and_maybe_bankrupt(game, player, delta)
      _emit_event(monopoly_event.chance.applied, {
        player = player,
        card = card,
