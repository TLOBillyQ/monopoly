...
local function _safe_query_start_pos(env, map_cfg)
  ...
  if not (lua_api and type(lua_api.query_unit) == "function") then
    return _zero_pos()
  end
  local path = map_cfg and map_cfg.path or nil
  local start_tile_id = type(path) == "table" and path[1] or nil
  if start_tile_id == nil then
    return _zero_pos()
  end
  local ok_unit, tile_unit = pcall(lua_api.query_unit, "t" .. tostring(start_tile_id))
  if not ok_unit or tile_unit == nil or type(tile_unit.get_position) ~= "function" then
    return _zero_pos()
  end
...
function synthetic_actor_registry.new(env)
  ...
  function registry.register_specs(specs)
    ...
    for _, spec in ipairs(specs) do
      registry.pending_specs[#registry.pending_specs + 1] = {
