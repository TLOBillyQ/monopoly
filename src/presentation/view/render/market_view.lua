...
local function _for_each_market_slot(callback)
  ...
  for index = 1, math.max(#buttons, #labels, #frames) do
    local button, label, frame = buttons[index], labels[index], frames[index]
...
local function _reset_market_preview(state)
  ...
  _set_market_preview_icon(state, _resolve_ref_key(image_refs, market_layout.empty_ref_key))
end
...
local function _filter_market_options(options)
...
local function _apply_market_common_controls(ui, market, confirm_enabled)
  _refresh_market_controls(ui, market)
  _set_confirm_button_state(ui, confirm_enabled)
  _set_cancel_controls(ui, market.allow_cancel, market.allow_cancel)
end
local function _resolve_selected_option(option_ids, selected_option_id, first_buyable)
...
function market_view.refresh_market_selection(state, option_id)
  ...
  _set_market_preview_icon(state, icon_key)
end
...
function market_view.refresh_market(state, market)
  ...
  local options = _filter_market_options(market.options)
  _set_market_container_active(ui, true)
  if #options == 0 then
    _hide_market_slots(ui)
    _reset_market_preview(state)
    _apply_market_common_controls(ui, market, false)
  ...
  _for_each_market_slot(function(idx, slot)
    local opt = options[idx]
    if opt and opt.can_buy == true and first_buyable == nil then
      first_buyable = opt.id or opt
    end
    option_ids[idx] = _set_market_slot(ui, refs, slot, opt)
  end)
  ui_controls.set_control_state(ui, market_layout.selected_card, { touch_enabled = false })
  _clear_market_selection_frames(ui)
  _apply_market_common_controls(ui, market, true)
  local selected = _resolve_selected_option(option_ids, market.selected_option_id, first_buyable)
