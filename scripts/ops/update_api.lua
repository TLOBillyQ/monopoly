...
local function _parse_args(args)
  ...
  while index <= #args do
    ...
    elseif token == "--help" or token == "-h" then
      print(_text(
        "用法: lua scripts/ops/update_api.lua [--old PATH] [--new PATH] [--doc-dir PATH] [--changelog PATH] [--limit NUM] [--skip-generate] [--skip-check] [--skip-diff]",
        "Usage: lua scripts/ops/update_api.lua [--old PATH] [--new PATH] [--doc-dir PATH] [--changelog PATH] [--limit NUM] [--skip-generate] [--skip-check] [--skip-diff]"
      ))
      os.exit(0)
    else
      _fail(_text(
...
local function _generate_docs(text, options)
  ...
  local index_lines = {
    ...
    "",
    "校验方式：运行 `lua scripts/ops/update_api.lua`，默认包含校验。",
    "",
  }
