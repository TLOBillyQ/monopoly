...
--- Set -Xoutput drop-info to suppress captured [info] diagnostics.
---
...
local function _has_output_argument(options, name)
  local arguments = options and options.arguments
...
return function(options)
  ...
  local drop_info = _has_output_argument(options, "drop-info")
  ...
  local function _process(captured)
    ...
    log_capture.replay(captured, function(line)
      local is_warn = line:find("%[warn%]", 1, false) ~= nil
      local is_dropped_info = drop_info and not _verbose and line:find("%[info%]", 1, false) ~= nil
      if not is_warn and not is_dropped_info then
        io.write("# " .. line .. "\n")
      end
    end)
  end
