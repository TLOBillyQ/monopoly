...
describe("landing", function()
  ...
  it("land_on_start_reward", function()
    ...
    local before = g:player_cash(p)
    local res = _resolve_landing(g, p, g.board:get_tile(idx), nil)
    assert(not res, "landing resolver should not wait")
    _assert_eq(g:player_cash(p), before + constants.pass_start_bonus, "landing on start should grant reward")
  end)
  ...
  it("land_on_start_reward_when_passed_start_zero", function()
    ...
    assert(not res, "landing resolver should not wait")
    _assert_eq(g:player_cash(p), before + constants.pass_start_bonus, "passed_start zero should still grant start reward")
  end)
  ...
  it("land_on_start_skips_duplicate_passed_start_reward", function()
    ...
    assert(not res, "landing resolver should not wait")
    _assert_eq(g:player_cash(p), before, "passed_start reward should not be duplicated on start landing")
  end)
  ...
  it("land_on_start_rich_deity_doubles_reward", function()
    ...
    assert(not res, "landing resolver should not wait")
    _assert_eq(g:player_cash(p), before + constants.pass_start_bonus * 2, "rich deity should double start reward")
  end)
  ...
  it("passing_player_with_steal_card_does_not_trigger_steal_and_still_pays_rent", function()
    ...
    g:update_player_position(p1, idx)
    local before_cash = g:player_cash(p1)
    ...
    assert(g:player_cash(p1) < before_cash, "rent must be paid after failed steal on AI-owned tile")
  end)
  ...
  it("landing_optional_waits_without_ui_and_can_resolve", function()
    ...
    assert(g:player_cash(p) < before_cash, "cash deducted for purchase")
  end)
