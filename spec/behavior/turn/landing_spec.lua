...
describe("landing", function()
  ...
  it("land_on_start_reward", function()
    ...
    local before = g:player_balance(p, "金币")
    local res = _resolve_landing(g, p, g.board:get_tile(idx), nil)
    assert(not res, "landing resolver should not wait")
    _assert_eq(g:player_balance(p, "金币"), before + constants.pass_start_bonus, "landing on start should grant reward")
  end)

  it("land_on_start_reward_when_passed_start_zero", function()
    local g = _new_game()
    local p = g:current_player()
    local idx, _ = _first_tile_by_type(g.board, "start")
    g:update_player_position(p, idx)
    local before = g:player_balance(p, "金币")
    local res = _resolve_landing(g, p, g.board:get_tile(idx), { passed_start = 0 })
    assert(not res, "landing resolver should not wait")
    _assert_eq(g:player_balance(p, "金币"), before + constants.pass_start_bonus, "passed_start zero should still grant start reward")
  end)

  it("land_on_start_skips_duplicate_passed_start_reward", function()
    local g = _new_game()
    local p = g:current_player()
    local idx, _ = _first_tile_by_type(g.board, "start")
    g:update_player_position(p, idx)
    local before = g:player_balance(p, "金币")
    local res = _resolve_landing(g, p, g.board:get_tile(idx), { passed_start = 1 })
    assert(not res, "landing resolver should not wait")
    _assert_eq(g:player_balance(p, "金币"), before, "passed_start reward should not be duplicated on start landing")
  end)

  it("land_on_start_rich_deity_doubles_reward", function()
    local g = _new_game()
    local p = g:current_player()
    ...
    assert(not res, "landing resolver should not wait")
    _assert_eq(g:player_balance(p, "金币"), before + constants.pass_start_bonus * 2, "rich deity should double start reward")
  end)
  ...
  it("passing_player_with_steal_card_does_not_trigger_steal_and_still_pays_rent", function()
    local g = _new_game({ ai = { [2] = true } })
    local p1 = g.players[1]
    local p2 = g.players[2]
    p1.inventory:add({ id = item_ids.steal })
    p2.inventory:add({ id = item_ids.roadblock })
    local idx, tile_ref = _first_land_tile(g.board)
    g:set_tile_owner(tile_ref, p2.id)
    g:set_player_property(p2, tile_ref.id, true)
    g:update_player_position(p1, idx)
    local before_cash = g:player_balance(p1, "金币")
    local before_steal_count = item_inventory.count(p1)
    local before_target_count = item_inventory.count(p2)
    local move_result = { encountered_players = { p2.id } }

    local res = _resolve_landing(g, p1, tile_ref, move_result)
    assert(not res, "passing another player should not open a landing choice")
    assert(_get_choice(g) == nil, "landing should not open steal prompt")
    _assert_eq(item_inventory.count(p1), before_steal_count, "steal card should remain unconsumed")
    _assert_eq(item_inventory.count(p2), before_target_count, "passed target should keep items")
    assert(g:player_balance(p1, "金币") < before_cash, "rent must be paid after failed steal on AI-owned tile")
  end)
  ...
  it("landing_optional_waits_without_ui_and_can_resolve", function()
    ...
    assert(g:player_balance(p, "金币") < before_cash, "cash deducted for purchase")
  end)
  ...
  it("mine_landing_defers_hospital_effect_until_move_followup", function()
    ...
    _assert_eq(g:detention_remaining(player), 0, "mine landing should not hospitalize before move followup")
    ...
    assert(g:detention_remaining(player) > 0, "mine move followup should apply hospital stay")
  end)
  ...
  it("legacy_save_with_seat_id_runs_through_mine_landing", function()
    ...
    assert(g:detention_remaining(player) > 0, "hospital stay must still apply for legacy save")
  ...
  it("inert_mine_does_not_trigger_on_landing", function()
    ...
    _assert_eq(g:detention_remaining(player), 0, "inert mine should not hospitalize anyone")
