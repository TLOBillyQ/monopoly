...
local _update_countdown_extended_tests = {
  ...
  function()
    ...
    game.turn = nil
    tick_ui_sync.update_countdown(game, state)
    assert(game.turn == nil, "nil turn should remain nil (no mutation)")
  end,
  function()
    local game = _new_game()
    local state = _build_loop_state()
...
local _update_countdown_final_tests = {
  function()
    ...
    tick_timeout.resolve_modal_gate = function()
    ...
    assert(modal_gate_calls >= 1, "popup branch should query modal_gate")
  end,
  function()
    local game = _new_game()
    local state = _build_loop_state()
