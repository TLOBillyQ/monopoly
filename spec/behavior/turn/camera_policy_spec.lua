...
describe("movement_camera", function()
  ...
  it("_test_sync_follow_refreshed_without_state_changes_target", function()
    local game = _new_game()
    local followed_id = nil
    local ports = {
      ui_sync = {
        follow_camera = function(state, player_id)
          assert(state == nil, "state should be optional when ui was refreshed")
    ...
    assert(followed_id == game.players[1].id, "ui_refreshed should allow target change without runtime state")
  end)
end)
