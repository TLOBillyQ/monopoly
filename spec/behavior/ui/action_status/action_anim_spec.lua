...
describe("presentation_action_anim_queue_and_turn_lock", function()
  ...
  it("_test_item_get_reveal_auto_hide_ignores_replaced_anim", function()
    local item_atlas_view = require("src.ui.render.item_atlas")
    local hidden = 0
    local scheduled = {}
    local state = {
    ...
    _with_patches({
      ...
      state.game.turn.action_anim = { seq = 13, kind = "item_get_reveal", player_id = 2 }
      scheduled[1]()
      _assert_eq(hidden, 0, "stale item reveal callback should not hide the active reveal")
    end)
  end)
  ...
  it("_test_role_control_lock_prunes_invalid_unseen_entries_without_error", function()
    local unit = _make_unit(0)
    local role = {
      get_roleid = function() return 1 end,
      get_ctrl_unit = function() return unit end,
    }
    local runtime = {
      for_each_role_or_global = function(fn)
        fn(role)
    ...
    _with_patches({
      ...
      local ok, err = pcall(role_control_lock_policy.sync, state, true, { runtime = runtime })
      assert(ok, tostring(err))
    end)

    assert(state.role_control_lock.by_role.stale_false == nil, "false stale entry should be pruned")
    assert(state.role_control_lock.by_role.stale_empty == nil, "empty stale entry should be pruned")
    assert(unit.add_calls == 1, "valid seen role should still be locked")
  end)
