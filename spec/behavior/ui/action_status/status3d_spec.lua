...
describe("presentation_status3d_and_turn_effects", function()
  it("_test_status3d_init_and_global_visibility", function()
  ...
  it("_test_status3d_priority_single_status", function()
    local env = _build_status3d_test_env()
    local state = {}
    local game = _build_status3d_game({
      tile_type = "hospital",
  ...
  it("_test_status3d_roadblock_only_current_turn", function()
    local env = _build_status3d_test_env()
    local state = {}
    local game = _build_status3d_game({
      last_turn = {
  ...
  it("_test_status3d_hospital_visible_when_no_action_notice_even_if_stay_turns_zero", function()
    local env = _build_status3d_test_env()
    local state = {}
    local game = _build_status3d_game({
      tile_type = "hospital",
  ...
  it("_test_status3d_hospital_visible_during_pending_location_effect", function()
    local env = _build_status3d_test_env()
    local state = {}
    local game = _build_status3d_game({
      tile_type = "hospital",
  ...
  it("_test_status3d_roadblock_anim_pending_overrides_hospital_pending", function()
    local env = _build_status3d_test_env()
    local state = {}
    local game = _build_status3d_game({
      tile_type = "hospital",
  ...
  it("_test_status3d_mountain_visible_when_no_action_notice_even_if_stay_turns_zero", function()
    local env = _build_status3d_test_env()
    local state = {}
    local game = _build_status3d_game({
      tile_type = "mountain",
  ...
  it("_test_status3d_hospital_mountain_not_visible_when_not_detained_and_stay_turns_zero", function()
    local prefab = require("Data.Prefab")
    local hospital_layout = prefab.scene_eui["医院状态"]
    local mountain_layout = prefab.scene_eui["深山状态"]
    local poor_layout = prefab.scene_eui["穷神状态"]
  ...
  it("_test_status3d_sync_layer_status_defaults_missing_remaining_to_zero", function()
    local calls = {}
    local cache = {
      layers = {
        [1] = {
          hospital = "hospital_layer",
        },
      },
      text_nodes = {
        [1] = {
    ...
    _with_patches({
      {
        target = status3d_scene,
        key = "resolve_observer_roles",
        value = function()
          return {
            {
              set_label_text = function(text_node, text)
                calls[#calls + 1] = text_node .. ":" .. text
              end,
            },
          }
        end,
      ...
      status3d_status.sync_layer_status(cache, player, "hospital", {})
    end)

    _assert_eq(#calls, 0, "missing stay_turns should default to zero and skip label")
  end)

  it("_test_status3d_sync_layer_status_caps_deity_remaining_only_above_cap", function()
    local calls = {}
    local cache = {
      layers = {
        [1] = {
    ...
    local player = {
      id = 1,
      deity_duration_turns = 3,
      status = {
        deity = { type = "rich", remaining = 3 },
      },
    }
    ...
    _with_patches({
      ...
      status3d_status.sync_layer_status(cache, player, "rich", {})
    end)

    _assert_eq(calls[1], "rich_text:剩余回合：3",
      "remaining equal to cap should render that remaining value")
  end)

  it("_test_status3d_sync_layer_status_defaults_missing_deity_remaining_to_zero", function()
    local calls = {}
    local cache = {
      layers = {
        [1] = {
    ...
    _assert_eq(#calls, 0, "missing deity remaining should default to zero and skip label")
  end)
  ...
  it("_test_turn_effects_prompt_visibility_follows_phase_and_role", function()
    ...
    _with_patches({
      ...
      _assert_eq(env.per_role_nodes[1]["基础_行动提示"].disabled, true, "current player action prompt should not consume item slot taps")
      _assert_eq(env.per_role_nodes[1]["基础_行动提示特效"].disabled, true, "current player action star should not consume item slot taps")
  ...
  it("_test_turn_effects_other_prompt_fallback_text", function()
    local env = _build_turn_effect_runtime_env({ 1, 2 })
    local state = {}
    local ui_model = {
      current_player_id = 1,
  ...
  it("_test_turn_effects_sync_restores_client_role_nil", function()
    local env = _build_turn_effect_runtime_env({ 1, 2 })
  ...
  it("_test_tick_ui_sync_turn_switch_still_follows", function()
  ...
  it("_test_tick_ui_sync_turn_switch_skip_follow_when_trigger_unavailable", function()
    local dirty_tracker = require("src.state.dirty_tracker")
    local main_view = require("src.ui.coord.ui_runtime")
    local ui_model = require("src.ui.view")
    local board_view_mod = require("src.ui.render.board")
  ...
  it("_test_ui_runtime_refresh_turn_label_toggles_countdown_nodes_and_label", function()
