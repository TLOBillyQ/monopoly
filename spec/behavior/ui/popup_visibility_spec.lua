...
describe("presentation.popup_visibility", function()
  it("_test_push_popup_sets_card_image_by_image_ref", function()
  ...
  it("_test_push_popup_hides_card_and_clears_image_when_missing", function()
    local last_image_key = nil
    local card_node = {
      set_texture_keep_size = function(_, image_key)
        last_image_key = image_key
  ...
  it("_test_popup_hidden_for_non_current_role", function()
  ...
  it("_test_card_popup_keeps_spectator_canvas_untouched", function()
    local state, _, query_nodes = _build_popup_view_state({
      ["Empty"] = "EMPTY",
    }, {
      set_texture_keep_size = function() end,
    ...
    assert(_has_event(role1_events, "显示卡牌展示屏"), "current role should see popup canvas")
    assert(not _has_event(role2_events, "显示卡牌展示屏"),
      "spectator must not be switched to popup canvas during chance/item card")
    assert(not _has_event(role2_events, "显示基础屏"),
      "spectator canvas must remain untouched so opened atlas stays visible")
    assert(not _has_event(role2_events, "隐藏道具图鉴"),
      "spectator atlas canvas must not be force-hidden during chance/item card popup")
  end)

  it("_test_bankruptcy_popup_visible_for_all_roles", function()
    local state, _, query_nodes = _build_popup_view_state({
      ["Empty"] = "EMPTY",
    }, {
      set_texture_keep_size = function() end,
  ...
  it("_test_bankruptcy_popup_avatar_uses_native_size_path", function()
  ...
  it("_test_resolve_bankruptcy_text_prefers_payload_text", function()
  ...
  it("_test_resolve_bankruptcy_text_falls_back_to_reason", function()
    local popup_controller = require("src.ui.coord.popup")
    local state, _, query_nodes = _build_popup_view_state({
      ["Empty"] = "EMPTY",
    }, {
  ...
  it("_test_resolve_bankruptcy_text_falls_back_to_player_name", function()
    local popup_controller = require("src.ui.coord.popup")
    local state, _, query_nodes = _build_popup_view_state({
      ["Empty"] = "EMPTY",
    }, {
  ...
  it("_test_resolve_bankruptcy_text_uses_default_when_all_missing", function()
    local popup_controller = require("src.ui.coord.popup")
    local state, _, query_nodes = _build_popup_view_state({
      ["Empty"] = "EMPTY",
    }, {
