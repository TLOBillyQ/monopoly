...
describe("gameplay rendering hotspots", function()
  ...
  it("resolve_player_status_key_nil_game_with_stay_turns_does_not_error", function()
    local player = {
      id = 1,
      position = 5,
      status = { stay_turns = 0 },
    ...
    assert(ok, "nil game should not raise while checking detention state")
    assert(key == nil, "nil game should not produce a location status")
  end)
  ...
  it("resolve_player_status_key_hospital_detained_wait_flag", function()
    local game = {
      board = {
        get_tile = function()
          return { type = "hospital" }
    ...
    local key = status3d_status.resolve_player_status_key(game, player)
    assert(key == "hospital", "detained_wait_active should keep location status visible")
  end)

  it("resolve_player_status_key_hospital_detained_wait_phase", function()
    local game = {
      board = {
        get_tile = function()
          return { type = "hospital" }
    ...
    local key = status3d_status.resolve_player_status_key(game, player)
    assert(key == "hospital", "detained_wait phase should keep location status visible")
  end)

  it("resolve_player_status_key_hospital_requires_matching_detention_turn", function()
    local base_game = {
      board = {
        get_tile = function()
          return { type = "hospital" }
    ...
    assert(status3d_status.resolve_player_status_key(base_game, player) == nil,
  ...
  it("resolve_player_status_key_hospital_without_turn_or_stay_turns_is_hidden", function()
    local game = {
      board = {
        get_tile = function()
          return { type = "hospital" }
    ...
    local key = status3d_status.resolve_player_status_key(game, player)
    assert(key == nil, "missing turn wait and stay_turns should not show hospital")
  end)
  ...
  it("resolve_player_status_key_poor_deity_without_remaining_is_hidden", function()
    local game = {
      board = {
        get_tile = function()
          return { type = "normal" }
    ...
    local key = status3d_status.resolve_player_status_key(game, player)
    assert(key == nil, "deity without remaining turns should not return a status key")
  end)
  ...
  it("status3d_init_sync_no_game_returns_early", function()
    ...
    assert(next(state) == nil, "no game should leave state untouched")
  end)
  ...
  it("status3d_init_sync_disabled_cache_returns_early", function()
    ...
    assert(state.ui_status_3d.disabled == true, "disabled cache should remain disabled")
    assert(next(state.ui_status_3d.layers) == nil, "disabled cache should not gain layers")
  end)
  ...
  it("status3d_init_sync_no_dirty_no_missing_layers", function()
    ...
    assert(state.ui_status_3d.layers.p1 ~= nil, "existing layer should be preserved on no-op sync")
  end)
