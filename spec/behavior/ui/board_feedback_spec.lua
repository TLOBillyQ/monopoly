...
describe("presentation.board_feedback", function()
  it("board_feedback_effect_id_ref_routes_integer_sfx_key", function()
    local effect_calls = {}
  ...
  it("board_feedback_tile_cue_uses_building_position_when_requested", function()
    local effect_calls = {}
  ...
  it("board_feedback_tile_cue_falls_back_to_tile_position_when_building_missing", function()
    local effect_calls = {}
  ...
  it("board_feedback_tile_cue_reads_host_unit_position", function()
    local effect_calls = {}
    local state = _build_state()
  ...
  it("board_feedback_effect_params_keep_payload_overrides_and_defaults", function()
    local effect_calls = {}
    local spawn_calls = {}
    local scaled_inputs = {}
    ...
    _with_patches({
      ...
      assert(played == true, "payload effect params should play")

      played = board_feedback.play_tile_cue(state, "upgrade_land_smoke", 1, {
        followup_sounds = {},
      })
      assert(played == true, "default effect params should play")
    end)

    assert(#effect_calls == 2, "effect params test should call engine twice")
    assert(scaled_inputs[1] == 2.0, "payload duration should be scaled from raw payload duration")
    assert(effect_calls[1].duration == 12.0, "payload duration should use scaled duration")
    assert(effect_calls[1].scale == 2.5, "payload scale should override cue scale")
    assert(effect_calls[1].rot == payload_rot, "payload rotation should override default rotation")
    assert(effect_calls[1].rate == 0.75, "payload rate should override default rate")
    assert(effect_calls[1].with_sound == true, "payload with_sound should override default")
    assert(spawn_calls[1].duration == 12.0, "effect track should receive scaled payload duration")

    assert(scaled_inputs[2] == 1.0, "cue duration should be scaled when payload duration is absent")
    assert(effect_calls[2].rot == runtime_constants.q_zero, "missing rotation should use default rotation")
    assert(effect_calls[2].rate == 1.0, "missing rate should use default rate")
    assert(effect_calls[2].with_sound == false, "missing with_sound should use default")
    assert(spawn_calls[2].duration == 11.0, "effect track should receive scaled default duration")
  end)

  it("board_feedback_effect_returns_false_when_host_runtime_cannot_play_sfx", function()
    local state = _build_state()
    ...
    assert(played == false, "effect-only cue should fail without a playable host runtime")
  end)

  it("board_feedback_does_not_bind_unbound_cue_to_player_unit", function()
    local bind_calls = 0
    local state = _build_state()
    ...
    _with_patches({
      ...
      assert(played == true, "unbound cue should still play")
    end)

    assert(bind_calls == 0, "cue without bind_to_player should not bind to the player unit")
  end)

  it("board_feedback_effect_only_failures_return_false", function()
    local play_calls = 0
    ...
    _with_patches({
      ...
      assert(played == false, "missing effect id on effect-only cue should return false")

      played = board_feedback.play_tile_cue(state, "item_stop_highlight", 1, {
        followup_sounds = {},
      })
      assert(played == false, "nil runtime sfx id on effect-only cue should return false")
    end)

    assert(play_calls == 1, "missing effect id should skip engine call before nil sfx handle case")
  end)

  it("board_feedback_player_effect_binding_keeps_bind_call", function()
    local effect_calls = {}
    local bind_calls = {}
    local state = _build_state()
    state.board_scene.units_by_player_id = {
  ...
  it("board_feedback_cash_burst_routes_scalar_scale", function()
    local effect_calls = {}
  ...
  it("board_feedback_player_cue_prefers_explicit_payload_position", function()
    local effect_calls = {}
  ...
  it("board_feedback_bankruptcy_routes_scalar_scale", function()
    local effect_calls = {}
  ...
  it("board_feedback_followup_sound_defaults_are_numeric", function()
    local sound_calls = {}
  ...
  it("board_feedback_unconfigured_effect_id_ref_skips_without_error", function()
    local play_calls = 0
  ...
  it("board_feedback_play_cue_with_nil_cue_name_returns_false", function()
    local effect_calls = {}
  ...
  it("board_feedback_play_cue_with_nil_pos_uses_default", function()
    local effect_calls = {}
  ...
  it("board_feedback_play_cue_both_effect_and_sound", function()
    local effect_calls = {}
    local sound_calls = {}
  ...
  it("board_feedback_play_cue_only_sound_no_effect", function()
    local effect_calls = {}
    local sound_calls = {}
  ...
  it("board_feedback_play_cue_effect_only_returns_true", function()
    local effect_calls = {}
  ...
  it("board_feedback_play_cue_followup_sound_with_delay", function()
    local sound_calls = {}
  ...
  it("board_feedback_play_cue_nil_payload_uses_defaults", function()
    local effect_calls = {}
