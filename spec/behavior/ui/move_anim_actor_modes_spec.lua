...
describe("presentation.move_anim_actor_modes", function()
  it("local_player_prefers_forced_move_stop_and_model_stop", function()
    local calls = {}
  ...
  it("synthetic_actor_uses_unified_move_start_and_stop", function()
    local calls = {}
    local scene = support.new_scene_with_linear_tiles(2, {
      units_by_player_id = {
        [-2] = {
  ...
  it("non_synthetic_actor_uses_regular_move_start", function()
    local calls = {}
    local scene = support.new_scene_with_linear_tiles(2, {
      units_by_player_id = {
        [-2] = {
  ...
  it("move_anim_debug_log_writes_when_enabled", function()
    local scene = support.new_scene_with_linear_tiles(2, {
      units_by_player_id = {
        [1] = {
    ...
    assert(string.find(text, "step_execute player_id=1 seq=31", 1, true) ~= nil, "debug log should include step execute seq")
    assert(string.find(text, "[MoveAnim] finish_stop", 1, true) ~= nil, "debug log should include finish stop")
  end)

  it("move_anim_debug_step_execute_logs_nil_seq_when_missing", function()
    local scene = support.new_scene_with_linear_tiles(2, {
      units_by_player_id = {
        [1] = {
          start_move_by_direction = function() end,
    ...
    local text = logger.get_text()
    assert(string.find(text, "step_execute player_id=1 seq=nil", 1, true) ~= nil,
