...
local function _test_roadblock_manual_choice_shows_seven_tiles_with_tile_names_only()
  ...
  for i, opt in ipairs(pending.options) do
    assert(expected_ids[opt.id] ~= nil, "roadblock option at " .. i .. " should be a valid candidate")
    _assert_eq(opt.label, expected_ids[opt.id], "roadblock option should show tile name only at " .. i)
  end

  assert(pending.target_slot_layout ~= nil, "roadblock choice should include target_slot_layout")
...
local function _test_roadblock_manual_choice_allows_current_tile()
  ...
  assert(self_option ~= nil, "center slot should have an option")
...
describe("item", function()
  ...
  it("item_phase_keeps_demolish_when_target_exists", function()
    local g = _new_game()
    local p = g:current_player()
    local idx = 3
    local tile_ref = g.board:get_tile(idx)
  ...
  it("item_phase_hides_rent_cards_on_owned_land", function()
    local g = _new_game()
    local p = g:current_player()
    local idx = 3
    local tile_ref = g.board:get_tile(idx)
  ...
  it("item_phase_hides_rent_cards_on_other_owned_land", function()
    local g = _new_game()
    local p = g:current_player()
    local idx = 3
    local tile_ref = g.board:get_tile(idx)
  ...
  it("build_wait_choice_args_requires_resume_next_state", function()
  ...
  it("build_wait_choice_args_allows_nil_resume_next_args", function()
  ...
  it("build_wait_choice_args_restores_next_state_and_args", function()
  ...
  it("steal_manual_uses_item_target_player_and_random_stolen_item", function()
    ...
    _assert_eq(g.turn.action_anim_queue[1].kind, "item_get_reveal", "steal reveal kind mismatch")
  ...
  it("item_executor_keeps_specific_anim_without_fallback", function()
    local g = _setup_world_with_anim()
    local p = g:current_player()
  ...
  it("item_phase_exposes_mine_in_pre_action", function()
    local g = _new_game()
    local p = g:current_player()
    p.inventory:add({ id = item_ids.mine })
  ...
  it("item_phase_exposes_mine_in_post_action", function()
    local g = _new_game()
    local p = g:current_player()
    p.inventory:add({ id = item_ids.mine })
  ...
  it("item_phase_exposes_send_poor_in_post_action", function()
    local g = _new_game()
    local p = g:current_player()
    p.inventory:add({ id = item_ids.send_poor })
  ...
  it("item_phase_dedupes_same_item_id_options", function()
    local g = _new_game()
    local p = g:current_player()
    p.inventory:add({ id = item_ids.remote_dice })
    p.inventory:add({ id = item_ids.remote_dice })
  ...
  it("item_strategy_hides_reactive_cards_from_active_window", function()
    local g = _new_game()
    local p = g:current_player()
    local idx = 3
    local tile_ref = g.board:get_tile(idx)
  ...
  it("item_phase_run_wait_action_anim_patches_move_followup_args", function()
    local g = _new_game()
    local player = g:current_player()
    player.auto = true
  ...
  it("collect_from_others_caps_fee_and_rich_bonus", function()
    local handlers = {}
    local events = {}
    local anims = {}
  ...
  it("roadblock_manual_choice_shows_seven_tiles_with_tile_names_only", _roadblock_and_demolish_tests[1])

  it("roadblock_manual_choice_allows_current_tile", _roadblock_and_demolish_tests[2])

  it("roadblock_manual_choice_hongkong_keeps_nearest_slots_ordered", _roadblock_and_demolish_tests[3])

  it("roadblock_manual_choice_hongkong_nearest_haikou_slot_places_correctly", _roadblock_and_demolish_tests[4])

  it("roadblock_manual_candidates_expose_nearest_unique_tiles_at_intersection", _roadblock_and_demolish_tests[5])

  it("roadblock_manual_candidates_use_shared_manhattan_range_at_branch", _roadblock_and_demolish_tests[6])

  it("demolish_manual_choice_uses_manhattan_range_at_branch", _roadblock_and_demolish_tests[7])

  it("roadblock_ai_uses_auto_candidates_only", _roadblock_and_demolish_tests[8])

  it("item_phase_select_remote_dice_keeps_followup_cancelable_in_repeatable_phase", function()
    local g = _new_game()
    local p = g:current_player()
    p.inventory:add({ id = item_ids.remote_dice })
  ...
  it("repeatable_phase_followup_cancel_reopens_item_phase_without_consuming", function()
    local g = _new_game()
    local p = g:current_player()
    p.inventory:add({ id = item_ids.remote_dice })
  ...
  it("repeatable_phase_followup_confirm_consumes_and_reopens_item_phase", function()
    local g = _new_game()
    local p = g:current_player()
    p.inventory:add({ id = item_ids.remote_dice })
    p.inventory:add({ id = item_ids.mine })
  ...
  it("preconsumed_followup_cancel_falls_back_to_first_option", function()
    local g = _new_game()
    local p = g:current_player()
  ...
  it("target_item_use_pushes_item_card_popup", function()
    local g = _new_game()
    local popups = {}
    local user = g.players[1]
    local target = g.players[2]
  ...
  it("remote_dice_followup_pushes_item_card_popup", function()
    local g = _new_game()
    local popups = {}
    local p = g:current_player()
    _set_ui_port(g, {
  ...
  it("steal_uses_tip_without_popup", function()
    local g = _new_game()
    local popups = {}
    local tips = {}
  ...
  it("steal_direct_success_queues_target_player_anim", function()
    local g = _setup_world_with_anim()
    local stealer = g.players[1]
    local target = g.players[2]
    stealer.inventory:add({ id = item_ids.steal })
    ...
    assert(g.turn.action_anim ~= nil, "steal should queue an action anim")
    ...
    _assert_eq(g.turn.action_anim_queue[1].kind, "item_get_reveal", "stolen reveal kind mismatch")
  ...
  it("steal_failure_uses_tip_without_popup", function()
    local g = _new_game()
    local popups = {}
    local tips = {}
    local stealer = g.players[1]
  ...
  it("steal_random_empty_target_fails_without_rng_or_consuming_card", function()
    local g = _new_game()
    local stealer = g.players[1]
    local target = g.players[2]
    stealer.inventory:add({ id = item_ids.steal })
  ...
  it("steal_with_full_inventory_fails_without_consuming_card", function()
    -- Per spec 1f825075 (features/game/items.feature L235-241): full-bag steal is
    -- a fail-with-toast path — card NOT consumed, target keeps the item.
    local g = _new_game()
    local stealer = g.players[1]
    local target = g.players[2]
    stealer.inventory:add({ id = item_ids.steal })
    ...
    _assert_eq(res and res.ok, false, "full-bag steal must report failure")
    _assert_eq(res and res.bag_full, true, "full-bag failure must surface bag_full flag")
    _assert_eq(stealer.inventory:count(), 5, "stealer inventory must stay at limit; card not consumed")
    assert(stealer.inventory:find_index(function(it)
      return it.id == item_ids.steal
    end) ~= nil, "steal card must remain in stealer's bag")
    _assert_eq(target.inventory:count(), 1, "target must still hold the item")
    assert(target.inventory:find_index(function(it)
      return it.id == item_ids.roadblock
    end) ~= nil, "target's roadblock must remain")
  end)

  it("rich_item_emits_deity_feedback_event", function()
    local g = _new_game()
    local p = g:current_player()
    local emitted = {}
  ...
  it("effect_pipeline_waiting_result_patches_followup_and_strips_intent", _effect_pipeline_tests[1])

  it("effect_pipeline_stop_if_short_circuits_before_optional_choice", _effect_pipeline_tests[2])

  it("effect_pipeline_single_optional_effect_uses_secondary_confirm_route", _effect_pipeline_tests[3])

  it("ai_can_use_item_returns_true_for_mine_in_pre_action_and_post_action", _effect_pipeline_tests[4])

  it("ai_can_use_item_uses_offer_in_phases_for_other_items", _effect_pipeline_tests[5])

  it("has_demolish_target_returns_true_when_target_exists", _effect_pipeline_tests[6])

  it("has_demolish_target_returns_false_when_no_target", _effect_pipeline_tests[7])

  it("has_target_player_returns_true_when_candidates_exist", _effect_pipeline_tests[8])

  it("has_target_player_returns_false_when_no_candidates", _effect_pipeline_tests[9])

  it("try_use_item_returns_nil_when_cond_fails", _effect_pipeline_tests[10])

  it("try_use_item_returns_nil_when_item_not_in_inventory", _effect_pipeline_tests[11])

  it("try_clear_obstacles_returns_result_when_obstacles_found", _effect_pipeline_tests[12])

  it("try_clear_obstacles_returns_nil_when_no_obstacles", _effect_pipeline_tests[13])

  it("try_remote_dice_returns_nil_when_no_dice_value_picked", _effect_pipeline_tests[14])

  it("try_roadblock_returns_nil_when_no_target_picked", _effect_pipeline_tests[15])
