...
describe("paid_currency", function()
  local _config_reset = require("spec.support.config_reset")
  before_each(function() _config_reset.reset_all() end)

  it("paid_bridge_external_currency_is_not_managed_and_setup_is_silent", function()
  ...
  it("external_paid_currency_still_starts_goods_purchase_panel", function()
    local game = _new_game()
    ...
    _with_currency_cfg({
      ...
      _with_patches(env.patch_list, function()
        ...
        local result = market.purchase.execute(game, p, 2009)
  ...
  it("market_paid_purchase_requires_goods_mapping", function()
    local game = _new_game()
    local p = game.players[1]
    local env = _build_fake_env(game, {
      goods_list = {
  ...
  it("market_paid_purchase_warns_missing_mapping_only_once_per_product", function()
    local game = _new_game()
    local p = game.players[1]
    local env = _build_fake_env(game, {
      goods_list = {
    ...
    local warns = _collect_warn_logs(function()
      _with_currency_cfg({
        ...
        _with_patches(env.patch_list, function()
          local market = _reload_market()
          local first = market.purchase.execute(game, p, 2009)
          local second = market.purchase.execute(game, p, 2009)
  ...
  it("hidden_paid_entries_do_not_log_mapping_missing", function()
    local game = _new_game()
    local p = game.players[1]
    local env = _build_fake_env(game, {
      goods_list = {
  ...
  it("paid_purchase_without_purchase_api_returns_explicit_reason", function()
    local game = _new_game()
    local p = game.players[1]
    local env = _build_fake_env(game)
  ...
  it("hidden_paid_product_is_rejected_before_mapping", function()
    ...
    local warns = _collect_warn_logs(function()
      _with_currency_cfg({
        ...
        _with_patches(env.patch_list, function()
          local market = _reload_market()
          local result = market.purchase.execute(game, p, hidden_entry.product_id)
  ...
  it("paid_purchase_callback_fulfills_item", function()
    local game = _new_game()
    local p = game.players[1]
    local env = _build_fake_env(game)
