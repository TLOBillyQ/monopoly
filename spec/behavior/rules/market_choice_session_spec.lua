...
describe("choices_session", function()
  ...
  it("_test_choice_session_apply_navigation_prev_next_and_rejects", function()
    ...
    _reload_module("src.rules.market.choice", {
      ...
      assert(choice.session.apply_navigation(game, pending_choice, { type = "market_page_prev" }) == true,
        "prev page should rebuild")
      assert(build_calls[1].page_index == 1, "prev page should decrement page index")
      assert(choice.session.apply_navigation(game, pending_choice, { type = "market_page_next" }) == true,
      ...
      assert(choice.session.apply_navigation(game, pending_choice, { type = "market_page_next" }) == false,
  ...
  it("_test_choice_session_refresh_after_paid_callback_rejects_non_owner_and_failed_rebuild", function()
    ...
    _reload_module("src.rules.market.choice", {
      ...
      local pending_choice = { kind = "market_buy", owner_role_id = 7, active_tab = "item", page_index = 1, page_count = 1 }
      local game = { dirty = {}, turn = { pending_choice = pending_choice } }
      assert(choice.session.refresh_after_paid_callback(game, { id = 8, name = "P8" }, { product_id = 2001 }) == false,
        "other player callback should be ignored")
      assert(choice.session.refresh_after_paid_callback(game, { id = 7, name = "P7" }, { product_id = 2001 }) == false,
    ...
    assert(#warnings == 1, "failed rebuild should emit only callback warning")
    assert(warnings[1]:find("market paid callback refresh skipped:", 1, true),
