...
- 行动日志屏（玩家可见事件流）的代码侧 API 已从 `debug_*` 改名为 `event_log_*`：`src/ui/ports/event_log.lua`、`src/ui/coord/event_log_view.lua`，方法 `set_event_log / set_event_log_visible / sync_event_log / resolve_event_log_enabled`。Eggy 画布节点名为 `日志屏`，自定义 UI 事件为 `显示日志屏` / `隐藏日志屏`，**但代码侧仍保持 event_log 命名**。
