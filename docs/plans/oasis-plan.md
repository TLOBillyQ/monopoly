...
- [ ] 抽出 eggy/love2d/oasis 的共同核心到 `src/adapters/core` 并改造 `src/adapters/eggy` 使用该核心，保证行为不变。
- [ ] 新建 `src/adapters/oasis` 适配层并接入 OASIS 的 BeginPlay、Tick、UI 事件入口。
- [ ] 增加 OASIS 适配层的最小 Lua 模拟测试与验证步骤。
...
首先抽取 `src/adapters/eggy/eggy_layer.lua` 与 `src/adapters/love2d` 中与平台无关的部分，形成共享适配核心，统一放在 `src/adapters/core`，避免 Eggy/Love2D/Oasis 重复实现。共享核心负责：游戏创建、回合推进、自动运行、待选择项的打开关闭、状态构建与日志输出。共享核心将依赖一个“UI 操作对象”，这个对象提供设置文字、按钮、可见性与触摸开关的方法，并持有 UI 节点命名规则。抽取后需要改写 `src/adapters/eggy/eggy_layer.lua` 与 `src/adapters/love2d` 相关文件只保留平台特定的事件绑定与 UI 对象实现，确保行为、文本与节点名保持完全一致。
...
1) 生成共享适配核心与改造 Eggy/Love2D 适配层。
   - 编辑 `src/adapters/eggy/eggy_layer.lua` 与 `src/adapters/love2d` 相关文件，把与平台无关的逻辑迁移到共享模块。
   - 新增共享模块到 `src/adapters/core`，并在 Eggy/Love2D/Oasis 适配层引用。
   - 运行依赖检查与回归测试：
