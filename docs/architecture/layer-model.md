...
`foundation` 是横切的基础设施基座（events / identity / log / number / tables / tips / ports），不含玩法语义。
...
```
src/foundation/
├── ports/         ← 横切端口（runtime_ports / action_anim / move_anim）
├── events.lua     ← monopoly_event 总线（前身 events/init.lua）
├── identity.lua   ← role_id 标识（前身 identity/）
├── log.lua        ← logger（前身 log/{logger,formatter,utils} 合并）
├── number.lua     ← 数字工具（前身 lang/number）
├── tables.lua     ← 表工具（前身 lang/tables）
└── tips.lua       ← tip_queue（前身 coordination/tip_queue）
```

> 2026-05-15 起 `refactor: foundation 子目录打平` (commit `e9e3ae1`) 把上述六个子目录合并为单文件层级。旧路径文档/grep 模式若仍指向 `src/foundation/{events,log,lang,identity,coordination}/` 需要替换。
...
- `src/foundation/ports/`：宿主/运行时广义契约（runtime_ports / action_anim / move_anim）
