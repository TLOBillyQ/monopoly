...
`scripts/arch/config.lua` 是这套分层模型的可执行投影。它把 `app`、`core`、`presentation`、`game_flow`、`game_systems`、`game_runtime`、`game_ai`、`state`、`infrastructure` 这些组件映射成 `arch_view` 的分类规则，再由 `lua scripts/arch.lua check` 对 `src/**/*.lua` 的静态 `require` 图做校验。这个工具只检查模块依赖图与循环基线；`lua scripts/arch.lua viewer --out-dir <dir>` 则把同一份投影导出为静态 viewer，支持层级 drill-down、dependency triangles、聚合 edge tooltip 与返回状态恢复。viewer 中的包目录节点应与 `init.lua` 包入口折叠成单一节点，避免出现 `foo` 与 `foo|file` 并列的重复展示。扩展扫描已合并到同一入口，直接执行 `lua scripts/arch.lua check`；若要扫描别的 Lua 项目，可显式传 `--project-root <dir> --config <file>`；若要复用已有扫描结果，可传 `viewer --in-json <file> --out-dir <dir> [--open]`。像旧路径禁用、宿主全局 API、runtime 对象字段直读这类文本护栏，仍留在 `tests/guards/dep_rules.lua`、`tests/guards/legacy_path_guard.lua`、`tests/guards/forbidden_globals.lua`；其中 `dep_rules.lua` 只保留硬边界，`legacy_path_guard.lua` 只保留 exact/prefix 退休路径回流检查。默认入口是 `lua tests/guard.lua`。
