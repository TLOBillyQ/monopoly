...
实现上，`scripts/crap.lua` 会转发到子模块 `vendor/crap4lua/`，自动注入 `scripts/quality/crap_monopoly.config.lua`，并通过 `scripts/quality/crap_monopoly_adapter.lua` 连接 Monopoly 的测试 lane。
