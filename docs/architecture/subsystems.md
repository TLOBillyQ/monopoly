...
| `src/host/` | Eggy 宿主实现：事件桥、运行时上下文、默认 runtime ports、seam exception | 游戏逻辑、UI 渲染 |
| `src/app/` | 装配：安装端口别名、读取启动参数 | 业务规则、宿主能力实现 |
| `src/ui/` | 展示共享 seam：`runtime_state` / `landing_visual_hold` / `host_bridge` 窄桥接 | controller 装配、render/input 逻辑 |
| `src/ui/ports/` | presentation grouped ports / runtime adapter 真源 | gameplay 规则、宿主底层实现 |
| `src/ui/schema/` | 纯展示 schema：节点名、contract、布局常量 | 状态写入、事件桥、输入语义 |
| `src/ui/input/` | 输入事件 → turn action 映射 | 业务推断 |
| `src/ui/model/` | UI model 构建与只读查询 | 写状态、触发副作用 |
| `src/ui/view/` | Canvas / widgets / render 输出 | 业务逻辑、输入处理 |
| `src/ui/` | 展示侧 runtime adapter、canvas store、UI 事件桥 | 游戏规则 |

## 放置决策速查

- 回合推进下一步 → `src/turn/`
- 玩法规则业务逻辑 → `src/rules/` 对应子目录
- ViewModel 渲染 → `src/ui/view/`
- 节点名 / 画布 contract / 布局常量 → `src/ui/schema/`
- 展示共享 seam / 运行时窄桥接 → `src/ui/`
- 展示 runtime grouped ports / adapter → `src/ui/ports/`
- 输入事件 → turn action → `src/ui/input/`
- 宿主能力接入端口 → `src/app/`
- 宿主能力实现 → `src/host/`
