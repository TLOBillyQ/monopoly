...
当前 Monopoly 只保留兼容入口壳；核心实现位于直接 clone 到仓库内的 `vendor/crap4lua/`。Monopoly 现在先通过公开 Lua runtime `crap4lua.bridge` 加载 `scripts/quality/crap/config.lua` 并执行 `scripts/quality/crap/adapter.lua` 收集 coverage，再把生成的 `ReportRequest` JSON 交给上游 CLI 完成报告分析与 viewer 导出。
...
`viewer --in-json` 直接渲染现有 JSON，不会加载 Monopoly 配置或测试 lane；`report` 会先 collect 再调用上游 CLI 的 `--request-json` 模式。缺少二进制时，包装层会在本地临时生成 launcher 并构建 `vendor/crap4lua/bin/crap4lua`，不会改写 vendor 目录内的上游源码。
