...
| `src/infrastructure/runtime` | 宿主细节：运行时上下文、事件桥、默认 runtime ports、显式 seam exception | — |
...
7. **loop_ports 拼装权限**：只有 `src/app/bootstrap/*`、`src/turn/loop/*` 与测试夹具可直接拼装 `loop_ports` override。
8. **架构边界可执行真源**：`tools/quality/arch/config.json`。
   ...
   - 工具实现位于子模块 `vendor/arch_view/`，viewer bundle 仅作为本地/CI 导出产物，不再提交到仓库
