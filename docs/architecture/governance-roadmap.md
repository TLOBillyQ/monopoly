...
> **See also**：架构决策 → [`../decisions/0001-seven-layer-with-foundation.md`](../decisions/0001-seven-layer-with-foundation.md)；分层模型 → [`layer-model.md`](layer-model.md)；目录边界 → [`boundaries.md`](boundaries.md)。
...
- **以物理目录名为真源对齐映射**：`src/host/`、`src/computer/`、`src/ui/` 继续视为物理真源；本轮治理聚焦修正文档与 `tools/quality/arch/config.json` 中的逻辑层映射、component 命名与例外注释，使其能无歧义映射回实际目录。
...
- **物理目录改名**。包括 `src/host/ → src/infrastructure/`、`src/computer/ → src/ai/`、`src/ui/ → src/presentation/`；本轮不以重命名目录来解决对齐债务。
...
| 项 | 完成标准 |
|----|---------|
| 文档落位 | `docs/architecture/governance-roadmap.md` 存在,且被 `AGENTS.md` "按任务找文档"表以独立行索引 |
| 决策点记录 | 3.1 主路径选择已记录为 D2(以物理目录名为准);`src/state/` 三处歧义点与 component 拆分决策仍在 Chapter 3 / Chapter 4 保留空白决策槽 |
...
### 3.1 主路径已拍板：D2（以物理目录名为准）

已拍板：本路线图按 **D2** 执行。`src/host/`、`src/ui/`、`src/computer/` 保持现名，后续只修正文档、映射表、component 定义与 exception 注释，使逻辑层描述能够稳定映射回物理目录。下表保留 D1/D2 对比，仅作为历史备选分析，不再作为当前执行分叉。
...
**Decision recorded**：已选 **D2**。若未来仍要推进目录改名，需另开 ADR / 新路线图，不在本文当前执行范围内。
...
**Action required from user**:在选项 A / B 之间裁定 component 模型;由于 3.1 已选 D2,若此处选 A,需额外说明为何物理目录不拆而 component 拆。
...
本表以 Chapter 3 各决策点的拍板结果为输入。由于 3.1 已明确选择 **D2（以物理目录名为准）**，当前执行时应以 **D2 路径下去向** 列为准；D1 列仅保留为历史备选分析，供未来另开目录改名 ADR 时参考。D1/D2 列之外的子决策组合差异在各列内联展开。
...
|---|------|----------|----------|----------|-------------------|--------------------------|
| 1 | `host_bridge_exception` | `config.json:18-22` | **当前 D2 主路径**:**保留并加注释**;仅当未来另开 D1 目录改名 ADR,且 `global_aliases` 不再需要跨层引用时 → **删除** | 当前 D2 下 `src/host/` 保持现名,但需确认 `src.host.global_aliases` 的消费方是否仍必须跨层访问;若未来改走 D1,则同时满足改名完成 + 消费方迁移后方可删除 | `governance_anchor`(指向本节 §5.1);`retain_reason`(当前以物理目录名为真源,`global_aliases` 仍是显式跨层 seam);`review_cadence`(每里程碑复审一次) | 3.1 |
| 2 | `infrastructure_runtime_bridges` | `config.json:24-31` | **当前 D2 主路径**:若 W3 + W4 完成后 infrastructure 不再引用 runtime 且 `rules.vehicle` 跨层引用消除 → **删除**;否则 **保留并加注释** | 3.2.2 选项决定 runtime_state 拆分方向;拆分完成后 infrastructure 层对 runtime 的引用路径是否已全部消除;`rules.vehicle` 是否已归入合规层 | `governance_anchor`(指向本节 §5.1);`retain_reason`(当前 D2 路径保留物理目录名,infrastructure component 仍需引用 runtime_refs/constants 或 `rules.vehicle`);`review_cadence`(每里程碑复审一次) | 3.1 + 3.2.2 |
...
| 5 | `state_access` | `config.json:48-58` | **按子决策分支逐条处置**:6 个模块中,按 3.2.2/3.2.3/3.4 各自迁移目标完成后,对应模块的 exception 条目 → **删除**;迁移未完成的模块条目 → **保留并加注释**(逐模块独立标注) | 各模块迁移状态独立判断:`deferred_dirty`、`landing_visual_hold`、`release_scheduler`、`runtime_state`、`ui_role_globals`、`vehicle_runtime_source` 是否已各自完成归类至其目标 component(`src/ui/` / `src/turn/` / `src/host/` / `src/player/` / `src/core/` 等) | `governance_anchor`(指向本节 §5.1,精确到模块名);`retain_reason`(具体模块迁移阻塞原因,一句话);`review_cadence`(每里程碑复审一次,逐模块独立记录) | 3.2.2 + 3.2.3 + 3.4 |
| 6 | `player_state_bridge` | `config.json:60-65` | **按子决策分支**:3.3 选 A 且 `player_state.lua` 已按 Chapter 4 实际迁出 `src/state/` → **删除**;3.3 选 B 或迁移未执行 → **保留并加注释** | 3.3 决策结果 + Chapter 4 对 `player_state.lua` 的实际迁移状态 | `governance_anchor`(指向本节 §5.1);`retain_reason`(3.3 选 B 或 `player_state.lua` 仍留在 `src/state/`,跨 component 归类仍是已知妥协);`review_cadence`(每里程碑复审一次) | 3.3 |

### 5.2 注释格式约定

保留 exception 时,必须在 `config.json` 对应条目内附带以下三个注释字段,字段名固定,值为单行字符串:

- `governance_anchor`:指向本路线图的精确锚点(格式:`docs/architecture/governance-roadmap.md#5-1`)
...
```jsonc
{
  ...
  // governance_anchor: docs/architecture/governance-roadmap.md#5-1
  // retain_reason: 当前以物理目录名为真源,global_aliases 仍是显式跨层 seam
...
|------|------|----------|----------|----------|----------|
| W1 | 决策落槽 | 本路线图已落位 | 确认 3.1 已选 D2,拍板 3.2.1-3.2.3、3.3、3.4;写入 ADR | 决策点之间耦合矛盾 | 人工 review |
...
| W5 | 物理目录改名（当前 D2 选型下跳过） | W1-W4 完成；仅当未来另开 ADR 选择 D1/混合策略时启用 | 当前路线图跳过；若未来重开目录改名，再单独执行 `git mv`、同步 `arch/config.json`、require、文档 | 混入本轮执行会把映射治理与目录重命名耦合 | `/verify-full`（仅未来启用时运行） |
...
**前置条件**:
- 本路线图(`docs/architecture/governance-roadmap.md`)已合并至主干
- Chapter 3 全部决策点已被相关责任人阅读

**主要动作**:
- 确认 3.1 已记录为 D2(以物理目录名为准);若后续要改为 D1/混合策略,必须另开 ADR / 新路线图,不得在本路线图内直接改向
...
- 将以上决策结果以"决策记录"形式追加至本文档附录或独立 ADR(`docs/decisions/`)
...
# 检查项:ADR 或附录中是否包含 3.1 已选 D2 + 3.2.1 / 3.2.2 / 3.2.3 / 3.3 / 3.4 的明确结论
```
...
**主要动作**:
- 打开 `docs/architecture/layer-model.md`,将 W1 决策结果写入对应章节(3.1 已选 D2、component 拆分结论、各文件归属)
- 在 `docs/architecture/layer-model.md` 中新增"逻辑层 → 物理目录"映射表,格式为:
...
- 若未来另开 ADR 改走 D1 或混合策略,再在映射表中标注哪些目录将进入目录改名波次
...
**前置条件**：
- W1-W4 全部完成
- 仅当未来另开 ADR 明确改为 D1 或混合策略时，本波次才启用；当前 3.1 已选 D2，因此本波次默认跳过并在附录留痕

**主要动作**：
- **当前决议（D2）**：本波次跳过；`src/host/`、`src/computer/`、`src/ui/` 保持现名，不执行 `git mv`
- 若未来另开 ADR 改走 D1 或混合策略，再按 W2 映射表，对需要改名的物理目录执行 `git mv`：
  - `src/host/` → `src/infrastructure/`（若适用）
  - `src/computer/` → `src/ai/`（若适用）
  - `src/ui/` → `src/presentation/`（若适用）
- 未来启用 W5 时，同步更新 `tools/quality/arch/config.json` 中所有涉及旧目录名的 `path` 字段
- 未来启用 W5 时，全局替换所有 `.lua` 文件中旧目录名的 `require` 路径
- 未来启用 W5 时，更新 `docs/architecture/layer-model.md`、`docs/architecture/boundaries.md` 中的目录引用
- 未来启用 W5 时，更新 `AGENTS.md` 中的目录说明（如有）
- 若未来启用 W5，配合 `.git-blame-ignore-revs` 维护改名提交，避免污染 blame 链
- 若未来启用 W5，运行完整车道验证
...
**完成判据**：
- 当前 D2 路径：附录已明确标注 W5 = skipped（reason: physical dir names are canonical），且 `src/host/`、`src/computer/`、`src/ui/` 仍与 W2 映射表一致
- 若未来启用 W5：旧目录名在 `src/` 下不再存在（`ls src/` 输出与 W2 映射表目标列一致）
- 若未来启用 W5：`config.json` 中无旧目录名引用
- 若未来启用 W5：`/verify-full` 全绿
- 若未来启用 W5：W2 映射表"物理目录（目标）"列全部标注为"已完成"
...
- 在以下文档顶部(导引段或第一个 `##` 标题之前)添加 "See also" 锚点,格式为:
  ...
  ```
  > **See also**: 治理路线图 → `docs/architecture/governance-roadmap.md`
  ```
...
- 在本文档(`governance-roadmap.md`)顶部导引段下方,将状态标注为:
...
- **OQ-2**:`src/host/global_aliases` 当前消费方清单是否完整?
  - 影响:W6 exception #1 处置;若未来另开 D1 目录改名 ADR,也会影响改名范围评估
  - 解决方式:由 W1 执行人在处理 exception #1 前用 `grep -rn 'global_aliases'` 全量扫描并记录,结果附于 Decision Record
...
- **OQ-4**:Eggy 宿主对接文档(`docs/reference/eggy/api/00_index.md` 及相关文件)是否直接引用 `src/host/` 路径?
  - 影响:R1 风险评估;若有硬编码路径引用,D1 改名需同步更新宿主文档
  - 解决方式:由 W1 执行人在改名前检索 `docs/reference/eggy/` 目录,结论写入 Decision Record
...
- **架构治理常态化机制**:本路线图为一次性治理行动。常态化机制(季度复审节奏、PR 模板架构合规检查项)的建立留作独立工作;`governance-roadmap.md` 纳入 `AGENTS.md` "按任务找文档"表的动作已在 W7 中安排(见附录 C)。
...
**选项**：D2（以物理目录名为准）  
**决策人**：user  
**决策依据**：对齐以 `src/host/`、`src/ui/`、`src/computer/` 等实际物理目录为真源，优先修正文档与 `tools/quality/arch/config.json` 的映射，不把目录改名与当前治理耦合。  
**已识别的耦合影响**：W5 目录改名波次默认跳过；Chapter 4 执行时只读 D2 列；后续若需改名须另开 ADR / 新路线图。  
**波次状态**：W5 = skipped（current D2 path）  
**预期完成波次**：W2 / W6 / W7
...
W7 必须在以下文件中插入或更新反向链接,指向 `docs/architecture/governance-roadmap.md`:

| 文件 | 插入位置 | 锚点格式 | 验证方式 |
|------|----------|----------|----------|
| `docs/architecture/layer-model.md` | 文件顶部"See also"节或末尾"参见"节 | `> **See also**: 治理路线图 → [governance-roadmap.md](governance-roadmap.md)` | `grep -n 'governance_roadmap' docs/architecture/layer-model.md` |
| `docs/architecture/boundaries.md` | 文件顶部"See also"节或末尾"相关文档"节 | `> **See also**: 治理路线图 → [governance-roadmap.md](governance-roadmap.md)` | `grep -n 'governance_roadmap' docs/architecture/boundaries.md` |
| `docs/architecture/health_signals.md` | "参考"或"相关"节 | `> **See also**: 治理路线图 → [governance-roadmap.md](governance-roadmap.md)` | `grep -n 'governance_roadmap' docs/architecture/health_signals.md` |
| `AGENTS.md` | "按任务找文档"表末行 | 追加一行:`\| 架构治理路线图 \| docs/architecture/governance-roadmap.md \|` | `grep -n 'governance_roadmap' AGENTS.md` |

> **AGENTS.md 说明**:在"按任务找文档"表中追加 `| 架构治理路线图 | docs/architecture/governance-roadmap.md |` 一行,使后续 agent 可通过任务类型直接定位本文档,无需全局搜索。
