...
```
lua5.4 tools/quality/coverage.lua --profiles=guards --threshold=0 --out=tmp/coverage_dryrun.md
```
单 profile 快速跑（适合调试）。多 profile 用逗号分隔。

```
lua5.4 tools/quality/coverage.lua --quiet
```
抑制 busted/luacov 进度日志，只输出错误与最终报告。

```
lua5.4 tools/quality/coverage.lua --reuse-stats --out=tmp/coverage_replay.md
```
跳过 busted 重跑，直接用现有 `luacov.stats.out` 重新生成报告。用于诊断报告解析问题或调参 threshold/out 时避免重复 30s 测试开销。

> **运行时绑定**：脚本必须在 Lua 5.4 下执行（项目主运行时）。busted/luacov shim 通过环境变量 `LUA54_BIN` / `BUSTED_BIN` / `LUACOV_BIN` 显式指定，未设置时按 `/opt/homebrew/bin/lua5.4` → `/usr/local/bin/lua5.4` → `/opt/homebrew/opt/lua@5.4/bin/lua5.4` → `/usr/local/opt/lua@5.4/bin/lua5.4` → PATH 顺序探测（Homebrew 的 `lua@5.4` 是 keg-only，默认不在 PATH，最后两条覆盖未 `brew link` 的情况），并通过 `luarocks --lua-version=5.4 list --porcelain` 解析 busted/luacov 安装位置。
...
| 场景 | 命令 |
|------|------|
| 提交前快速核查（仅 guards） | `lua5.4 tools/quality/coverage.lua --profiles=guards --threshold=0 --out=tmp/coverage_quick.md` |
| 完整 baseline | `lua5.4 tools/quality/coverage.lua --out=tmp/coverage_baseline.md` |
| 大重构后回归 | 比对前后两份 `tmp/coverage_*.md` 的 Per-Directory Summary |
| CI gate（≥90% 阈值生效后） | `lua5.4 tools/quality/coverage.lua` 或 `verify` step 7 |
...
| `Cannot locate lua5.4 binary` | 项目运行时未安装，或 keg-only 未 link | `brew install lua@5.4`（探测器已覆盖 keg 路径 `/opt/homebrew/opt/lua@5.4/bin/lua5.4`，无需 `brew link`）；或设置 `LUA54_BIN` 指向自定义二进制 |
| `Cannot locate busted/luacov for Lua 5.4` | rocks-5.4 缺包 | `luarocks --lua-version=5.4 install busted luacov luafilesystem` |
