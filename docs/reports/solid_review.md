...
审查范围聚焦于 `Components/` 与 `Manager/GameManager/` 的核心规则层、以及 `Manager/Adapter/Eggy/` 的适配层实现，结合现有文档与测试脚本输出，给出按 SOLID 原则的现状评估与改进建议。

## 总体结论

- 规则层（`Components/`、`Manager/GameManager/`）职责划分较清晰，流程驱动集中在 `TurnManager` 与 `ChoiceService`，可维护性较好。
- 适配层（`Manager/Adapter/`）已经拆分为 `AdapterLayer`、`Presenter`、Eggy 具体实现，但 `EggyLayer` 仍承担过多 UI/交互/动画/日志职责。
...
- `Components/Board.lua`、`Components/Store.lua`、`Components/Player.lua` 等聚焦单一模型职责。
- `Manager/GameManager/Movement/MovementService.lua` 只负责移动与路径计算；`Manager/GameManager/Land/LandActions.lua` 只负责地产相关动作。
- `Manager/Adapter/Core/Presenter.lua` 只负责把状态映射为视图结构。

**需要注意的部分**

- `Manager/GameManager/System/Game.lua` 同时负责状态写回、胜负判定、日志输出，存在职责混杂的风险。
- `Manager/Adapter/Eggy/EggyLayer.lua` 同时承担 UI 状态维护、事件分发、自动运行、动画驱动、日志与提示，后续扩展会变得困难。
- `Manager/Adapter/Core/AdapterLayer.lua` 既处理 choice/timeout 状态，又处理动画完成派发，职责偏多。

**建议**

- 明确 `Game` 只处理规则状态，胜负判定可拆到独立服务（例如 `victory_service.lua`）。
- 继续拆分 Eggy 适配层的子模块（目前已有 `Manager/Adapter/Eggy/EggyLayerBoard.lua`/`Manager/Adapter/Eggy/EggyLayerMarket.lua`），可以把“自动推进/动画”单独抽出。
...
- `TurnManager` 的 phases 在 `Manager/GameManager/System/CompositionRoot.lua` 中硬编码，新增回合阶段需要改动核心装配逻辑。
