...
当前导出同时包含小写蛇形与少量中文节点名（如“玩家1底板/玩家1底板颜色”），以 `Data/ui_data.lua` 为准。UI 资源侧需保持同名，避免中间映射层。
...
备注：当前 `ui_data.lua` 仅包含 `panel_player_2_land_count` 与 `panel_player_3_land_count`，其余玩家地块数量需在 Eggitor 侧补齐。
