...
| `choice/registry.lua`, `choice/item_preconsume_policy.lua`, `choice/use_skip_choice.lua`, `choice/resolver.lua` | rules (L6) | `src/rules/choice/` |
| `utils/dirty_tracker.lua`, `ui_sync_shared.lua` | state (L7) | `src/state/` |
| `utils/with_client_role.lua` | ui (L3) | `src/ui/utils/` |
| `utils/tip_queue.lua` | foundation（保留，见 plan.md 决策日志 Phase 3a 修订） | `src/foundation/coordination/tip_queue.lua` |
...
- 选定新名：**`visual_hold`**（候选 4 选 1 理由见 plan.md 决策日志）
- 留 state（L7），打包为子目录：
...
`src/ui/landing_visual_hold.lua`（4 行 wrapper）改名为 **`src/ui/visual_hold.lua`（顶层文件）** 而非 `src/ui/state/visual_hold.lua`：wrapper 同时引用 render（effect_track）与 state（impl），放在 state/ 子视图会引发 ui_state↔ui_render 子视图循环。顶层 ui/ 文件不参与子视图分类，无循环（详见 plan.md Phase 6 决策日志）。
...
| W3 拆分 component | 由 plan.md Phase 7b 执行 |
| W4 state/ 文件迁移 | 大半作废；只保留 D6 mixin 反转 + D5 visual_hold 打包 |
| W5 物理目录改名 | 不执行 |
| W6 Exception 清零 | 由 plan.md Phase 1 + Phase 7d 执行 |
| W7 反向链接 | 由 plan.md Phase 8d 执行 |
...
- **D3 + D4**：`tip_queue.lua` 从 foundation 移到 ui，但 `logger.lua` 在 foundation 引用 tip_queue。Phase 3a 执行 **Option C**：logger 改为 lazy require / DI 注入（plan.md 决策日志已记录）。
