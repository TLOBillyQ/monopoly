...
## 修订 (2026-07-07, 币种键收敛进 balance 深模块)

余额入口已从假多币种签名收敛为无币种参数的 cash 族，币种键 `"金币"` 不再出现在 `src/player/actions/balance.lua` 的公开接口上：

- `player_balance(player, "金币")` 已删除，读余额一律用 `player_cash(player)`；
- `deduct_player_balance(player, "金币", amount, opts)` 已删除，扣款一律用 `deduct_player_cash(player, amount, opts)`；
- `add_player_cash` / `set_player_cash` / `transfer_player_cash` 名称与语义不变。

本文正文与后果各节中 `player_balance("金币")` 的表述（背景、决策条款及“唯一合规读余额入口”的指示）自本修订起一律按 `player_cash` 理解；其余治理条款（`coin_count` 真源、硬失败、非负与有限整数校验、转账回滚、静态 guard 等）不变。

市场付费币种（金豆/乐园币）的识别仍在 commerce/market 侧（`paid_currency_bridge`）；本地收银路径对非金币币种保持硬失败（`src/rules/market/query.lua` 的 `assert_cash_currency`），不得静默按金币读扣。
