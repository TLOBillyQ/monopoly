...
## 2026-05-22 Post-flatten Addendum

Refactor `e9e3ae1`（2026-05-15，"foundation 子目录打平，消除 5 个嵌套层级"）将
`src/foundation/log/utils.lua` 与 `src/foundation/log/logger.lua` 等子目录文件
合并到顶层 `src/foundation/log.lua`。本 ADR 的不变量未变：foundation 仍不可
require state。仅路径替换如下。

| 历史路径（已不存在） | 现行路径 |
|-----------|-----------|
| `src/foundation/log/utils.lua` | `src/foundation/log.lua` |
| `src/foundation/log/logger.lua` | `src/foundation/log.lua` |

**完成判据**表中 line 132 的描述列改写为：

| 项 | 标准 |
|----|------|
| `src/foundation/log.lua` 不 require state | `grep "require.*src\.state" src/foundation/ --include='*.lua'` 应为 0 |

grep 范围（`src/foundation/`）从未改变，所以语义上 invariant 一直有效；只是描述
列的文件路径需要与代码现状对齐。

为防止未来再次回归，在 `spec/guards/lib/dep_rules.lua` 新增一条规则：
...
四条 pattern 分别覆盖：双引号 + 单引号 × 精确 `src.state` + 子模块 `src.state.X`。
匹配既有规则的引号穿透写法（见同文件 line 14-17、line 73-74）。

此规则覆盖整个 `src/foundation/` 目录，比单文件 grep 更稳健——任何未来新增到
foundation 的文件（无论是否 flat）只要试图 require `src.state`/`src.state.X`
都会被 guard lane 拦下。

历史叙事段落（lines 11/24/27/49/53/56）的 `src/foundation/log/utils.lua` 路径
保留不动：那是修复前的事件记录，按 ADR-as-history 惯例不可篡改。
