-- Shared helpers for regression specs (self-contained).
...
local turn_move = require("turn.step.walk")
local inventory = require("game.item.inventory")
local executor = require("game.item.executor")
local pricing = require("game.land.price")
local land_actions = require("game.land.action")
local steal = require("game.item.handler.steal")
local chance_effects = require("chance")
local landing_defs = require("cfg.LandingEffects")
local effect_pipeline = require("game.effect.pipeline")
local effect = require("game.effect.runner")
local choice_resolver = require("choice.resolve")
...
local with_patches = patch.with_patches
...
local function with_turn_flow(app_ctx)
  if not app_ctx.turn_flow then
    local phase_registry = require("game.core.phase")
    app_ctx.turn_flow = turn_flow:new(app_ctx, phase_registry.build_default_phases())
  end
  return app_ctx
end
