...
local function _test_monster_card()
  ...
  assert_eq(tile_state(g, tile_ref).level, 0, "building destroyed")
end
...
local function _test_missile_card()
  ...
  assert_eq(tile_state(g, tile_ref).level, 0, "building destroyed by missile")
...
local function _test_target_item_manual_direct_exec_and_duration()
  ...
  assert_eq(get_choice(g), nil, "choice should be resolved directly without reopening")
  assert_eq(user.cash, 5000, "manual target item should apply to user")
  assert_eq(target.cash, 5000, "manual target item should apply to target")
