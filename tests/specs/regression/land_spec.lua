...
local first_adjacent_land_pair = support.first_adjacent_land_pair
...
local function _test_ai_picks_land_purchase()
  ...
  local res = resolve_landing(g, ai_player, tile_ref, {})
  ...
  assert(tile_state(g, tile_ref).owner_id == ai_player.id, "land should be purchased")
end
...
local function _test_land_rent_contiguous_sum()
  ...
  local idx1, tile1, _, tile2 = first_adjacent_land_pair(g.board)
  ...
  assert_eq(before2 - tenant.cash, expected2, "contiguous rent sum after upgrade")
end
...
local function _test_rent_owner_missing_skips_payment()
  ...
  assert_eq(tenant.cash, before, "rent skipped when owner in mountain")
  ...
  assert_eq(ok, false, "execute_pay_rent should return false when owner missing")
  assert_eq(tenant.cash, before2, "rent skipped when owner missing")
end
