...
local turn_land = require("turn.step.arrive")
...
local function _test_land_on_start_reward()
  ...
  local idx, _ = first_tile_by_type(g.board, "start")
...
local function _test_pass_players_without_steal_does_not_crash()
  ...
  local res = resolve_landing(g, p1, g.board:get_tile(idx), {
    encountered_players = { p2.id },
  })
  assert(not res, "landing resolver should not wait without steal")
  assert(get_choice(g) == nil, "should not open choice without steal")
end
...
local function _test_landing_optional_waits_with_ui()
  ...
  g:update_player_position(p, idx)
  local res = resolve_landing(g, p, tile_ref, {})
...
local function _test_landing_optional_waits_without_ui_and_can_resolve()
  ...
  local resolved = resolve_choice_first(g, pending)
  assert(resolved, "expected at least one optional effect")
  assert(tile_state(g, tile_ref).owner_id == p.id, "land should be purchased after resolving choice")
...
local function _test_landing_optional_stale_choice_is_blocked()
  ...
  assert(tile_state(g, tile_ref).owner_id == nil, "stale buy_land should be blocked")
end
...
local function _test_turn_land_bridges_to_wait_action_anim_for_chance()
  local g = new_game()
  g.ui_port = build_ui_port({ wait_action_anim = true })
  local p = g:current_player()
  local idx, tile_ref = first_tile_by_type(g.board, "chance")
  g:update_player_position(p, idx)
...
local function _test_turn_land_bridges_to_wait_action_anim_for_item()
  ...
  g.ui_port = build_ui_port({ wait_action_anim = true })
  local p = g:current_player()
  local idx, tile_ref = first_tile_by_type(g.board, "item")
  g:update_player_position(p, idx)
  with_patches({
