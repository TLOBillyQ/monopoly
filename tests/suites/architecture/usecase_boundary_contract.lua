...
local function _test_choice_contract_copies_explicit_fields_once()
  ...
  _assert_eq(target.pre_confirm_on_select, false, "contract should copy select pre-confirm flag")
...
local function _test_output_state_adapter_exposes_invalidate_ui_model_only()
  local output_state_adapter = require("src.turn.output.state_adapter")
  local output = output_state_adapter.build_runtime_output_ports()
  local state = {}
  local changed = output.invalidate_ui_model(state)
  _assert_eq(changed, true, "runtime output.invalidate_ui_model should mark ui_runtime dirty")
  _assert_eq(state.ui_runtime and state.ui_runtime.ui_dirty, true, "invalidate_ui_model should write ui_runtime dirty state")
end
...
local function _test_gameplay_loop_output_port_override_requires_invalidate_ui_model()
  local calls = 0
  local resolved = gameplay_loop_ports.resolve({
    output = {
      invalidate_ui_model = function(state)
  ...
  local state = {}
  local changed = resolved.output.invalidate_ui_model(state)
  _assert_eq(changed, true, "invalidate_ui_model override should satisfy invalidate_ui_model")
  _assert_eq(calls, 1, "invalidate_ui_model override should be called once")
  _assert_eq(state.override_called, true, "invalidate_ui_model override should receive state")
end
...
return {
  ...
  tests = {
    ...
    { name = "output_state_adapter_exposes_invalidate_ui_model_only", run = _test_output_state_adapter_exposes_invalidate_ui_model_only },
    { name = "gameplay_loop_output_port_override_precedence", run = _test_gameplay_loop_output_port_override_precedence },
    { name = "gameplay_loop_output_port_override_requires_invalidate_ui_model", run = _test_gameplay_loop_output_port_override_requires_invalidate_ui_model },
