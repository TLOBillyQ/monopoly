...
local function _with_reloaded_move_module(movement_stub, followup_stub, fn)
  ...
  local original_followup = package.loaded["src.game.flow.turn.move_followup"]
  local original_move = package.loaded["src.game.flow.turn.move"]
  package.loaded["src.game.systems.movement"] = movement_stub
  package.loaded["src.game.flow.turn.move_followup"] = followup_stub
  package.loaded["src.game.flow.turn.move"] = nil
  local ok, result = pcall(function()
    return fn(require("src.game.flow.turn.move"))
  end)
  package.loaded["src.game.systems.movement"] = original_movement
  package.loaded["src.game.flow.turn.move_followup"] = original_followup
  package.loaded["src.game.flow.turn.move"] = original_move
  if not ok then
    error(result)
  end
  return result
...
_t2_case_groups.resolve_wait_state_tests = {
  function()
    ...
    local state_name = require("src.game.flow.turn.land")._resolve_wait_state(game, "post_action", { player = { id = 1 } }, true)
    assert(state_name == "wait_action_anim", "wait_action_anim should win when requested")
  end,
  function()
    local game = { turn = {}, dirty = {} }
    local state_name, args = require("src.game.flow.turn.land")._resolve_wait_state(game, "move", { player = { id = 1 } }, false)
...
return {
  ...
  _test_afk_auto_host_next_input_resets_timer = _test_afk_auto_host_next_input_resets_timer,
  _test_afk_auto_host_market_tab_input_resets_timer = _test_afk_auto_host_market_tab_input_resets_timer,
  _test_afk_auto_host_does_not_accumulate_when_input_locked = _test_afk_auto_host_does_not_accumulate_when_input_locked,
  _test_afk_auto_host_does_not_accumulate_when_popup_active = _test_afk_auto_host_does_not_accumulate_when_popup_active,
  _test_afk_auto_host_does_not_accumulate_in_wait_action_anim = _test_afk_auto_host_does_not_accumulate_in_wait_action_anim,
  _test_afk_auto_host_resets_when_current_player_changes = _test_afk_auto_host_resets_when_current_player_changes,
  _test_afk_auto_host_enters_auto_after_timeout_in_action_wait_phase = _test_afk_auto_host_enters_auto_after_timeout_in_action_wait_phase,
  _test_afk_auto_host_timeout_next_does_not_reset_timer = _test_afk_auto_host_timeout_next_does_not_reset_timer,
  _test_afk_auto_host_timeout_next_accumulates_across_turns = _test_afk_auto_host_timeout_next_accumulates_across_turns,
