...
local function test_session_script_run_to_finish()
  local visited = {}
...
local function test_session_script_resumes_from_current_state()
  local visited = {}
  local session = _make_session({
...
local function test_session_script_callable_table_handler()
  local call_count = 0
...
local function test_session_script_args_forwarded()
...
local function test_session_script_turn_mgr_passed_to_handler()
...
local function test_session_script_session_as_turn_mgr_fallback()
  local received_mgr = nil
  local session = _make_session({
    start = function(mgr, _) received_mgr = mgr; return nil end,
  })
...
return {
  name = "domain session_script coverage",
  tests = {
    { name = "session_script run to finish", run = test_session_script_run_to_finish },
    { name = "session_script resumes from current_state", run = test_session_script_resumes_from_current_state },
    { name = "session_script callable table handler", run = test_session_script_callable_table_handler },
    { name = "session_script args forwarded", run = test_session_script_args_forwarded },
    { name = "session_script turn_mgr passed to handler", run = test_session_script_turn_mgr_passed_to_handler },
    { name = "session_script session as turn_mgr fallback", run = test_session_script_session_as_turn_mgr_fallback },
  },
}
