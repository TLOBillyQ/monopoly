...
local function _test_free_rent_unavailable_on_non_property_tile()
...
local function _test_free_rent_unavailable_on_own_tile()
  _with_offer_phase_cfg({ [item_ids.free_rent] = true }, function()
    local g = _new_game()
    local p = g:current_player()
    local land_index, land_tile = support.first_land_tile(g.board)
...
local function _test_free_rent_available_on_rival_tile()
  _with_offer_phase_cfg({ [item_ids.free_rent] = true }, function()
    local g = _new_game()
...
local function _test_strong_unavailable_when_cash_is_below_total_invested()
...
local function _test_strong_available_when_cash_covers_total_invested()
  _with_offer_phase_cfg({ [item_ids.strong] = true }, function()
    local g = _new_game()
    local p, land_tile = _set_current_player_on_rival_land(g, 2)
    local total_invested = property_value.total_invested(land_tile, 2)
...
return {
  name = "item_availability_rent_response_crap_coverage",
  tests = {
    {
      name = "free_rent unavailable on non-property tile",
      run = _test_free_rent_unavailable_on_non_property_tile,
    },
    {
      name = "free_rent unavailable on own tile",
      run = _test_free_rent_unavailable_on_own_tile,
    },
    {
      name = "free_rent available on rival tile",
      run = _test_free_rent_available_on_rival_tile,
    },
    {
      name = "strong unavailable when cash is below total invested",
      run = _test_strong_unavailable_when_cash_is_below_total_invested,
    },
    {
      name = "strong available when cash covers total invested",
      run = _test_strong_available_when_cash_covers_total_invested,
    },
  },
}
