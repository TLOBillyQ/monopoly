...
local function _emit_resolve_event(kind, payload)
  if TriggerCustomEvent then
    TriggerCustomEvent(kind, payload or {})
  end
end
...
function chance_resolver.resolve(game, player, card, context)
  if card.negative and game:player_has_angel(player) then
    _emit_resolve_event(monopoly_event.chance.applied, {
      player = player,
      card = card,
      effect = card.effect,
...
local chance = {
  registry = chance_registry,
  resolve = chance_resolver.resolve,
}
