...
-- ==================== 地块表（基于map.md的9x9布局） ====================
-- map.md布局（列x，行y）：
-- 行1: 深山(1,1) 乌鲁木齐路(2,1) 兰州路(3,1) 呼和浩特路(4,1) 道具卡(5,1) 哈尔滨路(6,1) 长春路(7,1) 沈阳路(8,1) 税务局(9,1)
-- 行2: 拉萨路(1,2) [空] [空] [空] 银川路(5,2) [空] [空] [空] 石家庄路(9,2)
-- 行3: 西宁路(1,3) [空] [空] [空] 西安路(5,3) [空] [空] [空] 郑州路(9,3)
-- 行4: 成都路(1,4) [空] [空] [空] 太原路(5,4) [空] [空] [空] 合肥路(9,4)
-- 行5: 机会卡(1,5) 南昌路(2,5) 长沙路(3,5) 武汉路(4,5) 黑市(5,5) 机会卡(6,5) 北京路(7,5) 上海路(8,5) 机会卡(9,5)
-- 行6: 昆明路(1,6) [空] [空] [空] 天津路(5,6) [空] [空] [空] 济南路(9,6)
-- 行7: 贵阳路(1,7) [空] [空] [空] 重庆路(5,7) [空] [空] [空] 南京路(9,7)
-- 行8: 南宁路(1,8) [空] [空] [空] 机会卡(5,8) [空] [空] [空] 杭州路(9,8)
-- 行9: 医院(1,9) 澳门路(2,9) 香港路(3,9) 广州路(4,9) 道具卡(5,9) 海口路(6,9) 台北路(7,9) 福州路(8,9) 起点(9,9)
...
Config.chanceEvents = {
    ...
    {id = 3033, name = "税务局", type = "force_tax", value = 0, weight = 200, negative = true, target = "self", description = "你收到税务局通知，立刻赶往税务局交代问题。"},
...
Config.items = {
    -- 1级道具（基础卡）
    {id = 2001, name = "免费卡", level = 1, type = "free_pass", weight = 1000, immuneToAngel = false, 
     triggerTime = "after_action", description = "当你停留在其他玩家的地块上时，使用此卡可以免交本次租金。"},
    
    {id = 2002, name = "遥控骰子卡", level = 1, type = "remote_dice", weight = 1000, immuneToAngel = false,
     triggerTime = "before_action", description = "在你行动前可以使用，可以遥控骰子投出的点数。"},
    
    {id = 2003, name = "骰子加倍卡", level = 1, type = "dice_double", weight = 1000, immuneToAngel = false,
     triggerTime = "after_roll", description = "投出骰子后可以使用，使当前投出的点数加倍。"},
    
    {id = 2004, name = "路障卡", level = 1, type = "roadblock", weight = 1000, immuneToAngel = true,
     triggerTime = "active_use", description = "放置路障，任何玩家经过此地时强制停留1个回合。"},
    
    {id = 2005, name = "地雷卡", level = 1, type = "landmine", weight = 1000, immuneToAngel = true,
     triggerTime = "active_use", description = "在脚下放置地雷，任何玩家经过此地时触发地雷，摧毁座驾并强制住院。"},
    
    {id = 2006, name = "清障卡", level = 1, type = "clear_road", weight = 1000, immuneToAngel = false,
     triggerTime = "before_action", description = "放出机器人清除前方12格以内的路障和地雷。"},
    
    -- 2级道具（中级卡）
    {id = 2007, name = "偷窃卡", level = 2, type = "steal", weight = 500, immuneToAngel = true,
     triggerTime = "pass_player", description = "当你路过其他玩家时，可以选择使用此卡获得他的一个道具。"},
    
    {id = 2008, name = "怪兽卡", level = 2, type = "monster", weight = 500, immuneToAngel = false,
     triggerTime = "active_use", description = "选择前后3格内其他玩家的建筑，释放怪兽拆除该建筑。"},
    
    {id = 2009, name = "强征卡", level = 2, type = "force_acquire", weight = 500, immuneToAngel = false,
     triggerTime = "after_action", description = "停留在其他玩家地块上时，支付费用后强制获得这块地块的所有权。"},
    
    {id = 2010, name = "免税卡", level = 2, type = "tax_free", weight = 500, immuneToAngel = false,
     triggerTime = "tax_time", description = "在税务局征税时使用，可以抵扣本次税金。"},
    
    {id = 2011, name = "均富卡", level = 2, type = "equal_wealth", weight = 500, immuneToAngel = true,
     triggerTime = "active_use", description = "选择一个玩家，你和该玩家平分你们的总资金。"},
    
    {id = 2012, name = "流放卡", level = 2, type = "banish", weight = 500, immuneToAngel = true,
     triggerTime = "active_use", description = "选择一个玩家，将其强制流放到深山中。"},
    
    -- 3级道具（高级卡）
    {id = 2013, name = "导弹卡", level = 3, type = "missile", weight = 250, immuneToAngel = false,
     triggerTime = "active_use", description = "向前后3格范围内释放导弹，摧毁所有建筑和座驾，玩家住进医院。"},
    
    {id = 2014, name = "查税卡", level = 3, type = "tax_check", weight = 250, immuneToAngel = true,
     triggerTime = "active_use", description = "选择一个玩家，该玩家立即支付50%资金的所得税。"},
    
    {id = 2015, name = "请神卡", level = 3, type = "invoke_god", weight = 250, immuneToAngel = false,
     triggerTime = "active_use", description = "选择其他玩家，将其身上的附身神请到自己身上。"},
    
    {id = 2016, name = "送神卡", level = 3, type = "send_god", weight = 250, immuneToAngel = false,
     triggerTime = "when_cursed", description = "被穷神附身时使用，选择一个玩家，将穷神送到他身上。"},
    
    -- 特殊神仙卡（权重为0，只能通过机会卡获得）
    {id = 2017, name = "财神卡", level = 3, type = "wealth_god", weight = 0, immuneToAngel = false,
     triggerTime = "active_use", description = "财神附身5回合，收到的租金和奖金翻倍。"},
    
    {id = 2018, name = "穷神卡", level = 3, type = "poor_god", weight = 0, immuneToAngel = false,
     triggerTime = "active_use", description = "选择一个玩家，令其穷神附身5回合，支付的租金和罚金翻倍。"},
    
    {id = 2019, name = "天使卡", level = 3, type = "angel", weight = 0, immuneToAngel = false,
     triggerTime = "active_use", description = "天使附身5回合，免受负面卡牌效果影响。"}
}
...
Config.buildingCosts = {
    ...
    -- 升级到这个等级的费用 = 地块价格 * 系数
    multipliers = {1, 2, 4, 8}  -- 1级、2级、3级、4级的倍数
}
...
Config.constants = {
    -- 地图相关
    GRID_SIZE = 9,         -- 9x9网格
    TILE_COUNT = 45,       -- 45个地块（基于map.md布局）
    ...
    HOSPITAL_FEE = 1000,
    HOSPITAL_STAY = 2,
    HOSPITAL_POSITION = 14,  -- 医院所在格子
    
    -- 深山相关
    MOUNTAIN_STAY = 2,
    MOUNTAIN_POSITION = 15,  -- 深山所在格子
    
    -- 税务相关
    TAX_OFFICE_POSITION = 7,  -- 税务局所在格子
    TAX_RATE = 0.5,           -- 查税扣款比例
    
    -- 黑市相关
    BLACK_MARKET_POSITION = 13,  -- 黑市所在格子
    
    -- 监狱相关
    JAIL_POSITION = 16,       -- 监狱所在格子
    JAIL_STAY = 3,            -- 监狱停留回合数
