...
## 主要模块
- `src/app.lua`：游戏状态容器，持有玩家、棋盘、回合管理。
- `src/core/*`：核心数据结构（Board/Tile/Player/Dice 等）。
- `src/services/*`：业务规则（Movement/Tile/Item/Status/Turn 等）。
- `src/ui/*`：Love 渲染层，已拆为
  - `love_layer.lua`（主循环/输入），`layout.lua`（尺寸布局），`ui_state.lua`（主题/字体），
  - `board_renderer.lua`（棋盘/建筑/覆盖物/玩家），`panel_renderer.lua`（信息面板/日志），`modal.lua`（弹窗）。
...
- 脚本自检：`lua scripts/regression.lua`（纯 Lua，小型回归：经过起点、路障停留、怪兽卡/导弹卡效果）。
