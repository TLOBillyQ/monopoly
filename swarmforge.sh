...
prepare_worktrees() {
  ...
  for (( i = 1; i <= ${#ROLES[@]}; i++ )); do
    ...
    if [[ ! -e "$worktree_path/.git" && ! -d "$worktree_path/.git" ]]; then
      ...
      git -C "$worktree_path" submodule update --init --recursive >/dev/null 2>&1 \
        || echo "[swarmforge] WARN: submodule init failed for $worktree_path; run \`git -C $worktree_path submodule update --init --recursive\`" >&2
    fi
...
launch_role() {
  ...
  case "$agent" in
    claude)
      launch_cmd="export PATH='$SWARM_TOOLS_DIR:$SCRIPT_DIR':\$PATH && cd '$role_worktree' && claude --append-system-prompt-file '$prompt_file' --permission-mode bypassPermissions -n 'SwarmForge ${display}' \"\$(cat '$prompt_file')\""
      ;;
    codex)
      launch_cmd="export PATH='$SWARM_TOOLS_DIR:$SCRIPT_DIR':\$PATH && cd '$role_worktree' && codex --yolo -C '$role_worktree' \"\$(cat '$prompt_file')\""
      ;;
    copilot)
      launch_cmd="export PATH='$SWARM_TOOLS_DIR:$SCRIPT_DIR':\$PATH && cd '$role_worktree' && copilot --yolo -C '$role_worktree' --name 'SwarmForge ${display}' -i \"\$(cat '$prompt_file')\""
      ;;
    grok)
