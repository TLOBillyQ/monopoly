...
parse_config() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    case "$agent" in
      claude|codex|copilot|opencode|none) ;;
      *)
...
launch_role() {
  ...
  case "$agent" in
    claude)
      launch_cmd="export PATH='$SWARM_TOOLS_DIR:$SCRIPT_DIR':\$PATH && cd '$role_worktree' && claude --append-system-prompt-file '$prompt_file' --dangerously-skip-permissions -n 'SwarmForge ${display}' \"\$(cat '$prompt_file')\""
      ;;
    codex)
      launch_cmd="export PATH='$SWARM_TOOLS_DIR:$SCRIPT_DIR':\$PATH && cd '$role_worktree' && codex -C '$role_worktree' --approval-mode full-auto \"\$(cat '$prompt_file')\""
      ;;
    copilot)
      launch_cmd="export PATH='$SWARM_TOOLS_DIR:$SCRIPT_DIR':\$PATH && cd '$role_worktree' && copilot --yolo \"\$(cat '$prompt_file')\""
      ;;
    opencode)
      launch_cmd="export PATH='$SWARM_TOOLS_DIR:$SCRIPT_DIR':\$PATH && cd '$role_worktree' && opencode run \"\$(cat '$prompt_file')\""
      ;;
  esac
