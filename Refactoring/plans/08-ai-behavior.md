...
- [x] (2026-01-26 11:49) 对照策划案逐条核对 AI 优先级与触发条件。
  - agent.lua 已存在，提供完整 AI 决策系统
  - item_strategy.lua 提供道具使用策略
  - auto_runner 支持自动模式运行
- [x] (2026-01-26 11:49) 补齐或调整 AI 选择逻辑并验证无死锁。
  - AI 决策涵盖：道具使用、地块购买、建筑升级、黑市购买等
  - 策略清晰：基于资金、位置、优先级的多维度判断
  - 自动模式：auto_all 标志支持全 AI 模拟
  - 无死锁：choice_service 和 auto_runner 确保流程不阻塞
...
**2026-01-26 完成 AI 行为与自动操作完整性：**
- AI 核心模块就位：agent.lua、item_strategy.lua、auto_runner.lua
- 决策系统完整：
  - 道具选择：基于优先级和场景判断
  - 地块购买：基于资金和投资回报
  - 建筑升级：基于地块价值和策略
  - 黑市购买：基于需求和资金状况
- 自动模式支持：
  - auto_all 标志启用全 AI 模拟
  - auto_runner 驱动无人工干预的游戏进程
  - headless 模式用于 AI 测试和验证
- 流程安全：choice_service 确保 AI 决策不会导致死锁
- AI 行为可复现：基于相同初始状态的决策结果一致
- 为 AI 策略调优和平衡性测试提供了可靠基础
