...
## 6. Phase B 修复决策追加（2026-05）

### P1-6 决策：3017/3018 维持 negative=false

**决策**：保持现状，不改 chance_cards.lua 中 3017/3018 的 negative 字段。

**理由**：
- 3017 (台风) / 3018 (强制征地) 是 path-asset 效果，会摧毁/重置路径上**所有**地块（不分 owner）
- 如果改 negative=true，仅当**抽卡者本人**持 angel 时整张卡短路；无法保护 B/C 的 angel 玩家
- per-tile owner immunity 需要在 asset.lua handler 内逐 tile 检查 owner.angel_immune，工作量大且涉及"owner-side immunity"新语义
- 用户决定：实现与"风险声明 negative=false"已自洽（路过即损是无差别灾害），仅审计标记 SUSPICIOUS 已足够

**Follow-up backlog（不在本 Phase B 范围）**：
- 如未来需 per-tile owner immunity，可考虑：
  - asset.lua 内逐 tile 判 owner.angel_immune
  - 引入 `negative_to_owners` 细分类
  - 或拆 3017/3018 为多张卡（"摧毁自有"vs"摧毁所有"）
