...
local function hex_digit(ch)
  local b = string.byte(ch or "")
...
local function parse_hex4(text, pos)
  local value = 0
  for i = 0, 3 do
    local d = hex_digit(char_at(text, pos + i))
    if d == nil then build_error("invalid unicode escape", pos) end
    value = value * 16 + d
  end
  return value, pos + 4
end
  ...
  while cursor <= #text do
    ...
    if ch == "\\" then
      ...
      elseif next_ch == "u" then
        local cp, after = parse_hex4(text, cursor + 2)
        if cp >= 0xD800 and cp <= 0xDBFF then
          if char_at(text, after) == "\\" and char_at(text, after + 1) == "u" then
            local low, after2 = parse_hex4(text, after + 2)
