...
local function _windows_known_command_dirs()
  ...
  local function _push_env_relative(env_var, relative)
  ...
  _push_env_relative("ProgramFiles", "/Go/bin")
  _push_env_relative("ProgramFiles(x86)", "/Go/bin")
  _push_env_relative("USERPROFILE", "/scoop/apps/go/current/bin")
  _push("C:/Go/bin")
...
local function _bilingual_with_suffix(zh_prefix, en_prefix, suffix)
  local suffix_text = tostring(suffix)
  return common.bilingual(zh_prefix .. suffix_text, en_prefix .. suffix_text)
end
...
function common.ensure_dir(path)
  ...
  if not common.is_windows() then
    ...
    local success = _os_execute_success(ok, kind, code)
    if not success then
      return nil, _bilingual_with_suffix("创建目录失败: ", "Failed to create directory: ", path)
    end
    return true
  end
...
function common.write_file(path, content)
  ...
  if not common.is_windows() then
    ...
    if write_ok == nil then
      return nil, _bilingual_with_suffix("无法写入文件: ", "Cannot write file: ", path)
    end
    return true
  end
  ...
  if wrote_temp == nil then
    return nil, _bilingual_with_suffix("无法写入临时文件: ", "Cannot write temporary file: ", temp_path)
  end
...
function common.append_file(path, content)
  ...
  if not common.is_windows() then
    ...
    if write_ok == nil then
      return nil, _bilingual_with_suffix("无法追加文件: ", "Cannot append file: ", path)
    end
    return true
  end
...
function common.remove_path(path)
  ...
  if not common.is_windows() then
    ...
    local success = _os_execute_success(ok, kind, code)
    if not success then
      return nil, _bilingual_with_suffix("删除路径失败: ", "Failed to remove path: ", path)
    end
    return true
  end
...
function common.copy_tree(source_path, target_path)
  ...
  if not common.is_windows() then
    ...
    if not success then
      return nil, _bilingual_with_suffix("目录拷贝失败，退出码: ", "Copy tree failed with exit code: ", exit_code)
    end
    return true
  end
  ...
  local success = _os_execute_success(exec_ok, kind, code)
  if not success then
    return nil, _bilingual_with_suffix("目录拷贝失败: ", "Copy tree failed: ", source_path)
  end
  return true
end
...
function common.collect_files(root, extension)
  ...
  if not common.path_exists(normalized_root) then
    return nil, _bilingual_with_suffix("目录不存在: ", "Directory does not exist: ", root)
  end
  ...
  if not common.is_windows() then
    ...
    if not result.ok then
      return nil, _bilingual_with_suffix("收集文件失败: ", "Failed to collect files: ", root)
    end
...
function common.open_path(path)
  ...
  if not common.is_windows() then
    ...
    if not result.ok then
      return nil, _bilingual_with_suffix("打开路径失败: ", "Failed to open path: ", path)
    end
    return true
  end
