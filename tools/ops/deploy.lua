local _raw_script_path = arg and arg[0] or "tools/ops/deploy.lua"
...
local function _module_dir()
  local source = debug.getinfo(1, "S").source or "@tools/ops/deploy.lua"
...
local function _parse_args(args)
  ...
  while index <= #args do
    ...
    elseif token == "--help" or token == "-h" then
      print(_text(
        "用法: lua tools/ops/deploy.lua [--target-path PATH|-TargetPath PATH] [--startup-profile NAME|-StartupProfile NAME]",
        "Usage: lua tools/ops/deploy.lua [--target-path PATH|-TargetPath PATH] [--startup-profile NAME|-StartupProfile NAME]"
      ))
      os.exit(0)
    else
      _fail(_text(
