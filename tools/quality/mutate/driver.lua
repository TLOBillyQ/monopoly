...
local function _resolve_lane_suites(lane, opts)
  if lane == "behavior" then
    return catalog.load_behavior_suites(opts)
  end
  if lane == "contract" then
    return catalog.load_contract_suites(opts)
  end
  error("unsupported lane: " .. tostring(lane))
end
...
-- Parse the suite-list file once into a lookup of selected keys (or nil when no
-- file was given), so both the only-set builder and the suite filter share a
-- single read/parse instead of opening the file twice per run.
local function _resolve_selected_lookup(suite_list_file)
...
-- Build the set of suite keys (behavior: spec path, contract: module name) that a
-- filtered run cares about, so the catalog can load only those files instead of the
-- whole lane. Returns nil when nothing is selected (discovery / list / full run),
-- which tells the loader to load everything.
local function _build_only_set(suite_module, selected_lookup)
  if suite_module == nil and selected_lookup == nil then
    return nil
  end
  local only = {}
  if selected_lookup ~= nil then
    for key in pairs(selected_lookup) do
      only[key] = true
    end
  end
  if suite_module ~= nil then
    only[suite_module] = true
  end
  return only
end

local function _filter_suites(suites, suite_module, selected_lookup)
  if suite_module == nil and selected_lookup == nil then
    return suites
  end
...
function M.run(args, env)
  ...
  local selected_lookup = _resolve_selected_lookup(options.suite_list_file)
  local only = _build_only_set(options.suite_module, selected_lookup)
  local suites = resolve_lane_suites(options.lane, { only = only })
  suites = _filter_suites(suites, options.suite_module, selected_lookup)
  ...
  local run_ok, run_result = xpcall(function()
    local run_opts = {
      capture_logs = true,
      stop_on_first_failure = options.fail_fast,
