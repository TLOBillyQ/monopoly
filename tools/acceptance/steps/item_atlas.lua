...
local function _ensure_atlas_state(world)
  if not world.atlas_state then
    ...
    world.atlas_state = state
    world.atlas_visibility = captures.visibility
    world.atlas_labels = captures.labels
    world.atlas_textures = captures.textures
...
local SLOTS_PER_PAGE = item_atlas_nodes.page_size
...
local function _resolve_item(world, example, field)
  ...
  if not item then
    return nil, nil, nil, "no item at slot " .. tostring(slot)
  end
  return item, a, slot
end

local function _handler_node_visible(node, label, expect_visible)
  return function(world)
    local actual = world.atlas_visibility[node] == true
    if actual ~= expect_visible then
      local want = expect_visible and "visible" or "hidden"
      return nil, label .. " should be " .. want
    end
    return true
  end
end
...
function item_atlas_steps.handlers()
  return {
    -- ── catalog injection ─────────────────────────────────────────────────────
    ["道具目录共有<道具数>种道具"] = function(world, example)
      local n = number_utils.to_integer(example["道具数"])
      if n == nil then return nil, "invalid item count: " .. tostring(example["道具数"]) end
    ...
    ["当前页面展示<槽位数>个道具槽位"] = function(world, example)
    ...
    -- ── select slot ───────────────────────────────────────────────────────────
    ["玩家选中第<槽位>格道具"] = function(world, example)
    ...
    ["当前选中道具为第<槽位>格对应道具"] = function(world, example)
      local item, a, _, err = _resolve_item(world, example, "槽位")
    ...
    ["当前选中道具ID为<选中道具>"] = function(world, example)
      local expected_id = tostring(example["选中道具"] or "")
      local a = _atlas(world)
    ...
    ["放大卡牌显示第<槽位>格道具的名称"] = function(world, example)
      local item, a, _, err = _resolve_item(world, example, "槽位")
      if not item then return nil, err end
      if a.selected_item_id ~= item.id then
        return nil, "selected item mismatch: expected " .. tostring(item.id) .. ", got " .. tostring(a.selected_item_id)
    ...
    ["放大卡牌显示第<槽位>格道具的描述"] = function(world, example)
      local item, a, _, err = _resolve_item(world, example, "槽位")
      if not item then return nil, err end
      if a.selected_item_id ~= item.id then
        return nil, "selected item mismatch: expected " .. tostring(item.id) .. ", got " .. tostring(a.selected_item_id)
    ...
    ["图鉴关闭提示已展示"] = _handler_node_visible(item_atlas_nodes.close_hint_label, "图鉴_点击关闭提示", true),
    ["图鉴关闭提示已隐藏"] = _handler_node_visible(item_atlas_nodes.close_hint_label, "图鉴_点击关闭提示", false),
    ["图鉴空白关闭层已展示"] = _handler_node_visible(item_atlas_nodes.close_blank, "图鉴_点击空白关闭", true),
    ["图鉴空白关闭层已隐藏"] = _handler_node_visible(item_atlas_nodes.close_blank, "图鉴_点击空白关闭", false),
    ...
    ["图鉴上一页箭头已展示"] = _handler_node_visible(item_atlas_nodes.page_prev, "图鉴_上一页", true),
    ["图鉴上一页箭头已隐藏"] = _handler_node_visible(item_atlas_nodes.page_prev, "图鉴_上一页", false),
    ["图鉴下一页箭头已展示"] = _handler_node_visible(item_atlas_nodes.page_next, "图鉴_下一页", true),
    ["图鉴下一页箭头已隐藏"] = _handler_node_visible(item_atlas_nodes.page_next, "图鉴_下一页", false),
    ...
    ["图鉴总页数为<总页数>"] = function(world, example)
    ...
    ["道具目录末页展示<末页槽位数>个道具槽位"] = function(_, example)
    ...
    ["图鉴打开角色ID为<验证角色ID>"] = function(world, example)
      ...
      if atlas.role_id ~= expected then
        return nil, "expected atlas role_id=" .. tostring(expected) .. ", got " .. tostring(atlas.role_id)
      end
      return true
    end,
    ...
    ["当前图鉴页码为<页码>"] = function(world, example)
    ...
    ["图鉴卡片渲染数为<卡片渲染数>个"] = function(world, example)
    ...
    -- ── 卡牌槽位贴图断言 ─────────────────────────────────────────────────────
    ["图鉴卡牌槽位<槽位>当前贴图为道具<道具ID>的图"] = function(world, example)
      local slot, err = _parse_slot(example, "槽位")
      if slot == nil then return nil, err end
      local expected_item_id = tostring(example["道具ID"] or "")
      if expected_item_id == "" then return nil, "missing 道具ID" end
      _ensure_atlas_state(world)
      local node_name = item_atlas_nodes.card_images[slot]
      if node_name == nil then
        return nil, "no card image node for slot " .. tostring(slot)
      end
      local refs = world.atlas_state.ui_refs.images
      local expected_key = refs[expected_item_id]
      if expected_key == nil then
        return nil, "no image ref seeded for item " .. expected_item_id
      end
      local actual_key = world.atlas_textures[node_name]
      if actual_key ~= expected_key then
        return nil, "slot " .. tostring(slot) .. " texture mismatch: expected "
          .. tostring(expected_key) .. ", got " .. tostring(actual_key)
      end
      return true
    end,
