...
function dice_steps.handlers()
  return {
    ...
    ["玩家的骰子数量为<骰子数>"] = function(world, example)
      local count = number_utils.to_integer(example["骰子数"])
      if count == nil then
        return nil, "invalid dice count: " .. tostring(example["骰子数"])
      end
    ...
    ["每颗骰子的结果在<最小值>到<最大值>之间"] = function(world, example)
      local min_val = number_utils.to_integer(example["最小值"])
      local max_val = number_utils.to_integer(example["最大值"])
    ...
    ["掷出骰子数为<验证骰子数>颗"] = function(world, example)
      local expected = number_utils.to_integer(example["验证骰子数"])
      if expected == nil then
        return nil, "invalid 验证骰子数: " .. tostring(example["验证骰子数"])
      end
      local rolls = game_driver.last_rolls(_ctx(world))
      if #rolls ~= expected then
        return nil, "expected " .. tostring(expected) ..
          " dice rolled, got " .. tostring(#rolls)
      end
      return true
    end,
    ...
    ["玩家使用了遥控骰子设定点数为<设定值>"] = function(world, example)
      local val = number_utils.to_integer(example["设定值"])
      if val == nil then
        return nil, "invalid remote dice value: " .. tostring(example["设定值"])
      end
      game_driver.apply_remote_dice(_ctx(world), _player(world), world.dice_count, val)
      return true
    end,

    ["每颗骰子结果均为<设定值>"] = function(world, example)
      local expected = number_utils.to_integer(example["设定值"])
      local rolls = game_driver.last_rolls(_ctx(world))
      for i, result in ipairs(rolls) do
    ...
    ["移动步数为<总步数>"] = function(world, example)
      local expected = number_utils.to_integer(example["总步数"])
      local total = game_driver.last_total(_ctx(world))
      if total ~= expected then
    ...
    ["玩家持有骰子加倍卡且倍率为<倍率>"] = function(world, example)
      local mult = number_utils.to_integer(example["倍率"])
      if mult == nil then
        return nil, "invalid multiplier: " .. tostring(example["倍率"])
      end
      game_driver.set_dice_multiplier(_ctx(world), _player(world), mult)
      return true
    end,

    ["玩家掷骰得到原始点数<原始点数>"] = function(world, example)
      local raw = number_utils.to_integer(example["原始点数"])
      if raw == nil then
    ...
    ["实际移动步数为<实际步数>"] = function(world, example)
      local expected = number_utils.to_integer(example["实际步数"])
      local total = game_driver.last_total(_ctx(world))
      if total ~= expected then
