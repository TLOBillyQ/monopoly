local _raw_script_path = arg and arg[0] or "tools/data/export_xlsx.lua"
...
local function _module_dir()
  local source = debug.getinfo(1, "S").source or "@tools/data/export_xlsx.lua"
  local normalized = _normalize_path(source):gsub("^@", "")
  return normalized:match("^(.*)/[^/]+$") or "tools/data"
end
...
local function _parse_args(args)
  ...
  while index <= #args do
    ...
    elseif token == "--help" or token == "-h" then
      print(_text(
        "用法: lua tools/data/export_xlsx.lua [--output-dir OUTDIR]",
        "Usage: lua tools/data/export_xlsx.lua [--output-dir OUTDIR]"
      ))
      os.exit(0)
    else
      _fail(_text(
