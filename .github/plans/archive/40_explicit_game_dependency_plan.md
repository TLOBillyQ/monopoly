...
移除 GameplayLoop 中通过 state 间接持有 game 的做法，改为显式传入或直接引用 game；同时评估 Manager 目录内的依赖注入模式，改为显式依赖暴露，避免通过 setup/deps 注入。用户视角行为不变：回合推进、动画等待、choice/popup 流程、自动行动与 UI 事件路由保持一致。验证方式是运行 .github/tests/regression.lua 并观察现有回归脚本全部通过。
...
- [x] (2026-02-01 15:55+08:00) 自测并记录证据（.github/tests/regression.lua）
