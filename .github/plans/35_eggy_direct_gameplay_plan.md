...
- [x] (2026-01-31 15:07Z) 删除 Runtime 层文件与引用；tests/regression.lua 已通过。

## 意外与发现


观察：tests/regression.lua 已补回并改为调用 GameplayLoop。
...
第一步：列出 Runtime 相关引用，形成修改清单。
  ...
  预期：输出只包含 init.lua、Manager/System/Runtime.lua、Manager/System/GUI/RuntimeLoop.lua、Manager/System/GUI/RuntimeUI.lua、Manager/TurnManager/GUI/UIEventRouter.lua 与 tests/regression.lua。
...
本重构是纯代码迁移，改动可重复执行。若发现 UI 或回合推进异常，可临时回退到原 Runtime 层：恢复 Manager/System/Runtime.lua 与 Manager/System/GUI 目录，并把 init.lua 恢复为调用 Runtime.install；UIEventRouter 与 tests/regression.lua 恢复原 require 即可。完成后应确保删除的 Runtime 文件不再被引用。
...
GameplayLoop.lua 需要导出以下接口，供 init.lua、UIEventRouter 与 tests/regression.lua 调用：
