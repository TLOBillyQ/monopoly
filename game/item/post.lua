  ...
  [item_ids.tax] = {
    apply = function(game, user, target, context)
      ...
      if game:player_balance(target, "金币") <= 0 then
        bankrupt.eliminate(game, target, { reason = target.name .. " 支付查税费用后破产" })
      end
      return true
    end,
  },
...
local function _handle_clear_obstacles_ahead(game, player, cfg, context)
  ...
  utils.queue_walk(queue, function(node, push)
...
function post.get_target_spec(item_id)
  return target_effects[item_id]
end

function post.target_item_ids()
  return target_item_order
end

function post.apply_target(game, user, item_id, target, context)
...
function post.apply_post(game, player, item_id, context)
  context = context or {}
