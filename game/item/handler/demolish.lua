...
local function _destroy_building(game, tile_obj)
  assert(tile_obj ~= nil and tile_obj.type == "land", "invalid tile for demolish")
  game:set_tile_level(tile_obj, 0)
end
...
function demolish.find_target(game, player, distance)
  local idx, value = utils.find_best_tile(game, player, distance, {
    score_fn = function(tile_obj)
      if tile_obj.type ~= "land" then
        return -1
      end
      local st = tile_state(game, tile_obj)
      if not st.owner_id or st.owner_id == player.id or (st.level or 0) <= 0 then
        return -1
      end
      return utils.total_invested(tile_obj, st.level)
    end,
  })
  if value < 0 then
    return nil
...
function demolish.apply(game, player, idx, opts)
  ...
  local tile_obj = assert(game.board:get_tile(idx), "missing tile: " .. tostring(idx))

  _destroy_building(game, tile_obj)
  ...
  local msg
  if opts.injure then
    msg = player.name .. " 发射导弹轰炸 " .. tile_obj.name
    if tile_obj.type == "land" then
       msg = msg .. "，建筑被摧毁"
    end
    if hit > 0 then
    ...
    msg = player.name .. " 释放怪兽拆毁 " .. tile_obj.name .. " 的建筑"
  end
...
function demolish.use(game, player, distance, consume_fn, opts)
  ...
  if not opts.by_ai then
    local idxs = utils.indices_in_range(game.board, player.position, distance)
    local options = {}
    local body_lines = {}
      ...
      if idx and idx ~= player.position then
        ...
        if tile_obj.type == "land" then
          ...
          if st.owner_id and st.owner_id ~= player.id and st.level > 0 then
            table.insert(body_lines, "#" .. idx .. " " .. tile_obj.name)
            table.insert(options, { id = idx, label = tile_obj.name })
          end
        end
      end
    end
