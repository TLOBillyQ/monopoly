...
local roadblock = require("game.item.handler.roadblock")
local demolish = require("game.item.handler.demolish")
...
local function _current_rent(tile, level)
  return price.rent_for_level(tile, level or 0)
end
...
local function _simulate_landing(game, player, steps)
  ...
  for step = 1, steps do
    ...
    local tile_obj = board:get_tile(current)
    if tile_obj and tile_obj.type == "market" and step < steps then
      break
    end
  end
...
local function _remote_priority(game, player, sim)
  local tile_obj = sim.tile
  if not tile_obj then
    return nil
  end
  local st = nil
  if tile_obj.type == "land" then
    st = tile_state(game, tile_obj)
  end
  local rank, score
  if tile_obj.type == "item" then
    rank, score = 1, sim.steps
  elseif tile_obj.type == "chance" then
    rank, score = 2, sim.steps
  elseif tile_obj.type == "land" and st and not st.owner_id then
    rank, score = 3, sim.steps
  elseif tile_obj.type == "land" and st and st.owner_id == player.id then
    rank, score = 4, sim.steps
  elseif tile_obj.type == "start" then
    rank, score = 5, sim.steps
  elseif tile_obj.type == "market" then
    rank, score = 6, sim.steps
  elseif tile_obj.type == "mountain" then
    rank, score = 7, sim.steps
  elseif tile_obj.type == "tax" then
    rank, score = 8, sim.steps
  elseif tile_obj.type == "hospital" then
    rank, score = 9, sim.steps
  elseif tile_obj.type == "land" and st and st.owner_id and st.owner_id ~= player.id then
    rank, score = 10, -_current_rent(tile_obj, st.level)
  end
  if not rank then
    return nil
  end
...
function target.pick_remote_dice_value(game, player, dice_count)
...
function target.pick_target_player(game, player, item_id, options)
...
function target.pick_roadblock_target(game, player)
...
function target.pick_demolish_target(game, player, distance)
