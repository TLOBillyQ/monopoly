...
local function _total_assets(game, player)
  ...
  for tile_id in pairs(player.properties) do
    ...
    assert(tile_obj ~= nil and tile_obj.type == "land", "invalid property tile: " .. tostring(tile_id))
    local st = tile_state(game, tile_obj)
    local level = st.level
    total = total + price.total_invested(tile_obj, level)
  end
  return total
end
...
local function _apply_winners(game, winners, message)
  ...
  event.emit(event.game.finished, {
    winners = winners,
...
function win.check_victory(self)
