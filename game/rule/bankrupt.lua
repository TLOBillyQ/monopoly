...
local function _collect_owned_tiles(game, player)
  ...
  for _, tile_id in ipairs(owned_tile_ids) do
    ...
    if tile_obj then
      table.insert(owned_tiles, tile_obj)
      table.insert(names, tile_obj.name)
    else
      logger.warn("bankrupt skip missing tile:", tostring(tile_id))
    end
  end
...
function bankrupt.eliminate(game, player, opts)
  ...
  if #owned_tile_ids > 0 then
    ...
    for _, tile_obj in ipairs(owned_tiles) do
      game:reset_tile(tile_obj)
      game:set_player_property(player, tile_obj.id, false)
    end
  end
