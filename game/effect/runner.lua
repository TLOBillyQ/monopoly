...
local runner = Class("EffectRunner")
...
local function _resolve_executor(effect_id, game_ctx)
  ...
  return assert(registry:get(effect_id), "missing exec: " .. tostring(effect_id))
end
...
function runner.scan(effect_defs, player, tile, game_ctx)
...
function runner.execute(eff, player, tile, game_ctx)
  assert(game_ctx ~= nil, "missing game_ctx")
  local ctx = _build_ctx(player, tile, game_ctx)
  local ok, reason = _can_apply(eff, ctx, game_ctx)
  if not ok then
...
function runner.build_game_ctx(game, move_result, opts)
  opts = opts or {}
