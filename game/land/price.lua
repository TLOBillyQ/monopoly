...
function price.max_level(tile)
  return _max_level(tile)
end

function price.purchase_price(tile)
  return tile.price
end

function price.upgrade_cost(tile, level)
  local costs = tile.upgrade_costs
  if type(costs) ~= "table" then
    return 0
  end
...
function price.rent_for_level(tile, level)
...
function price.total_invested(tile, level)
  local total = price.purchase_price(tile)
  local costs = tile.upgrade_costs
  if type(costs) ~= "table" then
    return total
  end
