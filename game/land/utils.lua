...
function utils.queue_walk(queue, visit)
  local q = queue or {}
  local head = 1
  while head <= #q do
    local node = q[head]
...
function utils.indices_in_range(board, start, distance)
...
function utils.total_invested(tile, level)
  assert(tile ~= nil, "missing tile")
  return price.total_invested(tile, level or 0)
end

function utils.find_best_tile(game, player, distance, opts)
  ...
  local indices = utils.indices_in_range(board, player.position, distance or 3)
  if allow_self then
  ...
  for _, idx in ipairs(indices) do
    if allow_self or idx ~= player.position then
      ...
      local value = assert(score_fn(tile_obj, idx), "missing score for tile: " .. tostring(idx))
      if has_best then
