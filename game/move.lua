...
local function _check_roadblock(board, current, player)
  ...
  _emit_event(game_event.movement.roadblock_hit, {
    player = player,
...
local function _check_steal(player, encountered_step, step, abs_steps, facing, branch_parity, opts)
  ...
  _emit_event(game_event.movement.steal_interrupt, {
    player = player,
...
local function _check_market(board, current, step, steps, abs_steps, facing, branch_parity, player, opts)
  ...
  assert(tile_obj ~= nil, "missing tile: " .. tostring(current))
  if tile_obj.type ~= "market" or step >= steps then
    return nil
  end
  local remaining = abs_steps - step
  _emit_event(game_event.movement.market_interrupt, {
    player = player,
    remaining_steps = remaining,
...
function move.move(game, player, steps, opts)
  opts = opts or {}
  ...
  _emit_event(game_event.movement.moved, {
    player = player,
  ...
  if pass_start > 0 then
    ...
    _emit_event(game_event.movement.passed_start, {
      player = player,
      count = pass_start,
      bonus = bonus,
