...
func RunIndexSuites(projectRoot, driverScript, lane, mode string, jsonOutput bool) (string, int, error) {
	...
	projectHashValue, err := mutproject.ProjectHashForIndex(projectRoot)
	if err != nil {
		return "", 1, err
	}
	if existing, err := loadSuiteIndex(projectRoot, lane, mode, projectHashValue); err == nil && existing != nil {
		path := suiteIndexPath(projectRoot, lane, mode, projectHashValue)
		if jsonOutput {
			payload, _ := mutruntime.JSONString(map[string]any{"ok": true, "path": mutruntime.NormalizePath(path), "project_hash": projectHashValue, "suite_count": len(existing.Suites)})
			return payload + "\n", 0, nil
		}
		return fmt.Sprintf("Indexed %d suites for %s (%s)\n", len(existing.Suites), lane, projectHashValue), 0, nil
	} else if err != nil {
		return "", 1, err
	}
	args := []string{"lua", Path(projectRoot, driverScript), "--lane", lane, "--emit-suite-file-map-json", "--quiet"}
	if mode != "" {
		args = append(args, "--mode", mode)
	}
	result := mutruntime.RunCommand(projectRoot, args, 0, false)
	if result.ExitCode != 0 {
		return "", 1, fmt.Errorf("index-suites failed: %s", result.Output)
	}
	...
	for suite, files := range payload.SuiteFiles {
		...
		for _, file := range files {
			normalizedFiles = append(normalizedFiles, mutruntime.NormalizeRelativePath(file))
		}
		sort.Strings(normalizedFiles)
		index.Suites[suite] = mutruntime.DedupeStrings(normalizedFiles)
	}
	path := suiteIndexPath(projectRoot, lane, mode, projectHashValue)
